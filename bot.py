...
def main_menu_kb(is_admin=False):
    rows = [[
        ...
        rbtn(bt("install"), style="success", emoji_id=get_eid("install"),
             web_app_url="https://azizbekqiyomov5555-arch.github.io/Kkk/"),
    ], [
...
def _check_ffmpeg() -> bool:
    """ffmpeg o'rnatilganligini tekshiradi."""
    try:
...
def _build_watermark_filter(user_id: str) -> str:
    """
    Video ichida 6 ta turli joyda paydo bo'lib yo'qoluvchi watermark filtri.
    Har 3 soniyada: 2 soniya ko'rinadi, 1 soniya ko'rinmaydi.
    Har bir watermark boshqa vaqtda va joyda.
    """
    wm_text = f"O\\'g\\'irlash taqiqlanadi | ID\\: {user_id}"
    base_style = (
    ...
    # 6 ta watermark — turli joylarda va turli vaqtlarda paydo bo'ladi
    positions = [
        # (x_expr, y_expr, time_offset)
        ("(main_w-text_w)/2",             "(main_h-text_h)/2",             0),    # Markazda
        ("main_w*0.05",                    "main_h*0.05",                   0.5),  # Yuqori chap
        ("main_w-text_w-main_w*0.05",      "main_h*0.05",                   1.0),  # Yuqori o'ng
        ("main_w*0.05",                    "main_h-text_h-main_h*0.08",     1.5),  # Pastki chap
        ("main_w-text_w-main_w*0.05",      "main_h-text_h-main_h*0.08",     2.0),  # Pastki o'ng
        ("(main_w-text_w)/2",              "main_h*0.05",                   2.5),  # Yuqori markazda
    ]

    filters = []
    for x, y, offset in positions:
        # Har 4 soniyada: 2 soniya ko'rinadi, 2 soniya ko'rinmaydi
        alpha_expr = f"if(lt(mod(t+{offset},4),2),0.85,0)"
        f = (
...
def _add_watermark_ffmpeg(input_path: str, output_path: str, user_id: str) -> bool:
    """ffmpeg orqali videoga watermark qo'shadi. True qaytarsa — muvaffaqiyatli."""
    try:
        vf_filter = _build_watermark_filter(str(user_id))
        cmd = [
            "ffmpeg", "-y",
            "-i", input_path,
            "-vf", vf_filter,
        ...
        result = subprocess.run(
            cmd, capture_output=True, timeout=300  # 5 daqiqa max
        )
        if result.returncode != 0:
            logger.error(f"ffmpeg xato: {result.stderr.decode('utf-8', errors='replace')[:500]}")
            return False
        return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_inline_rows.append([
        ibtn(bt("install"), web_app_url="https://azizbekqiyomov5555-arch.github.io/Kkk/",

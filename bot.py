...
# Asosiy (parent) bot admini — bola botlar ham shu ID ga muhim xabarlarni yuboradi
PARENT_ADMIN_ID = int(os.environ.get("PARENT_ADMIN_ID") or "8537782289")
# Asosiy bot tokeni: bola botlardan keladigan yechish so'rovlari shu bot orqali asosiy adminga boradi.
# Railway Variables ichida PARENT_BOT_TOKEN qilib kiriting; kodga tokenni yozmang.
PARENT_BOT_TOKEN = os.environ.get("PARENT_BOT_TOKEN") or os.environ.get("MAIN_BOT_TOKEN") or ""
...
# ─── TELEGRAM API (my.telegram.org) — LOCAL BOT API + Pyrogram MTProto uchun ─────
API_ID   = int(os.environ.get("API_ID")   or "3736974")
...
# ─── BOLA BOTNING TOKENI ORQALI XABAR YUBORISH (raw Bot API) ──
def _child_bot_send_sync(token: str, method: str, payload: dict):
    url = f"https://api.telegram.org/bot{token}/{method}"
    try:
        r = requests.post(url, json=payload, timeout=30)
        data = r.json()
    except Exception as e:
        return False, f"HTTP xato: {e}"
    if not data.get("ok"):
        return False, data.get("description") or "noma'lum xato"
    return True, None

def _child_bot_send_file_sync(token: str, method: str, field: str,
                              file_tuple, data: dict):
    url = f"https://api.telegram.org/bot{token}/{method}"
    try:
        r = requests.post(url, data=data, files={field: file_tuple}, timeout=120)
        j = r.json()
    except Exception as e:
        return False, f"HTTP xato: {e}"
    if not j.get("ok"):
        return False, j.get("description") or "noma'lum xato"
    return True, None

async def notify_parent_admin(bot, text: str, parse_mode: str = "HTML") -> bool:
    """Xabarni asosiy bot tokeni orqali asosiy adminga yuboradi.

    Bola botda context.bot ishlatilsa xabar bola bot nomidan ketadi. PARENT_BOT_TOKEN
    berilgan bo'lsa, raw Telegram Bot API orqali aynan asosiy bot nomidan yuboramiz.
    """
    payload = {"chat_id": PARENT_ADMIN_ID, "text": text}
    if parse_mode:
        payload["parse_mode"] = parse_mode

    if PARENT_BOT_TOKEN:
        ok, err = await asyncio.to_thread(_child_bot_send_sync, PARENT_BOT_TOKEN, "sendMessage", payload)
        if ok:
            return True
        logger.error(f"parent bot sendMessage failed: {err}")

    # Fallback: PARENT_BOT_TOKEN kiritilmagan yoki xato bo'lsa, eski usul bilan urinadi.
    try:
        await bot.send_message(PARENT_ADMIN_ID, text, parse_mode=parse_mode)
        return True
    except Exception as e:
        logger.error(f"fallback parent admin send failed: {e}")
        return False

async def _child_bot_send(token: str, method: str, payload: dict):
    return await asyncio.to_thread(_child_bot_send_sync, token, method, payload)

async def _child_bot_send_file(token: str, method: str, field: str,
                               file_tuple, data: dict):
    return await asyncio.to_thread(_child_bot_send_file_sync, token, method,
                                   field, file_tuple, data)
...
def factory_spawn(row: dict) -> bool:
    ...
    env["PARENT_ADMIN_ID"] = str(PARENT_ADMIN_ID)
    if PARENT_BOT_TOKEN:
        env["PARENT_BOT_TOKEN"] = PARENT_BOT_TOKEN
...
class RamCache:
    ...
    def __init__(self):
        ...
        # ✅ AUTO TO'LOV STATISTIKASI — faqat CheckCard (auto) orqali tushgan pul yig'iladi
        self.auto_topup_total: int = 0
        self.auto_topup_count: int = 0
    ...
    def to_dict(self) -> dict:
        with self._lock:
            return {
                ...
                "auto_topup_total": int(self.auto_topup_total or 0),
                "auto_topup_count": int(self.auto_topup_count or 0),
            }
    ...
    def from_dict(self, data: dict):
        ...
        with self._lock:
            ...
            try:
                self.auto_topup_total = int(data.get("auto_topup_total") or 0)
            except Exception:
                self.auto_topup_total = 0
            try:
                self.auto_topup_count = int(data.get("auto_topup_count") or 0)
            except Exception:
                self.auto_topup_count = 0
            self.loaded           = True
...
DEFAULT_BTN = {
    ...
    "auto_topup_stat":    _B("💰 Auto to'lov tushgan pul"),
    "topup_auto":         _B("Humo Uzcard"),
    "topup_manual":       _B("Odiy to'lov"),
    ...
    "child_msg":          _B("📨 Bola botga xabar yuborish"),
    "post_nomi":          "🎭",
    "post_qism":          "🎞",
    "post_kod":           "🔑",
    "post_janr":          "🎬",
...
BTN_LABELS["auto_topup_stat"] = "Auto to'lov tushgan pul (balans)"
...
def _load_parent_emoji_ids() -> dict | None:
    """Bola-bot uchun: asosiy (parent) bot bazasidan emoji_ids ni o'qiydi.
    Parent ma'lumotlari default 'public' schema da yotadi; bola-bot esa
    o'z 'child_*' schema'siga search_path bilan bog'lanadi. Shu sababli
    parent ga to'g'ridan-to'g'ri public schema bilan ulanamiz."""
    if not DATABASE_URL or not PSYCOPG2_AVAILABLE:
        return None
    try:
        u = _fx_urlparse(DATABASE_URL)
        q = dict(_fx_parse_qsl(u.query))
        q["options"] = "-c search_path=public"
        parent_url = _fx_urlunparse(u._replace(query=_fx_urlencode(q, quote_via=_fx_quote)))
        conn = _pg_connect(parent_url)
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT value FROM public.bot_data WHERE key = 'main'")
                row = cur.fetchone()
        finally:
            try: conn.close()
            except: pass
        ...
        if not isinstance(data, dict):
            return None
        eids = data.get("emoji_ids") or {}
        # Faqat raqamli (premium custom emoji) IDlarni qaytaramiz
        return {k: str(v).strip() for k, v in eids.items() if str(v).strip().isdigit()}
    except Exception as e:
        logger.error(f"_load_parent_emoji_ids xato: {e}")
        return None
...
def db_initial_load():
    ...
    if IS_CHILD_BOT:
        ...
        # ─── Asosiy (parent) botdan premium emoji ID larni olib kelamiz ───
        try:
            parent_emojis = _load_parent_emoji_ids()
            if parent_emojis:
                changed = False
                for _k, _v in parent_emojis.items():
                    _vs = str(_v).strip()
                    if _vs.isdigit() and RAM.emoji_ids.get(_k) != _vs:
                        RAM.emoji_ids[_k] = _vs
                        changed = True
                if changed:
                    logger.info(f"✨ Parent bot dan {len(parent_emojis)} ta premium emoji ID ko'chirildi")
                    try:
                        _save_postgres(RAM.to_dict(), retries=1)
                    except Exception as _se:
                        logger.error(f"emoji_ids save xato: {_se}")
        except Exception as _pe:
            logger.error(f"Parent emoji_ids olishda xato: {_pe}")
        EMOJI_IDS.clear()
...
def register_user(user):
    ...
    if uid_str not in RAM.users:
        ...
        # eski user — phone yo'q bo'lsa, kalit qo'shamiz
        if "phone" not in RAM.users[uid_str]:
            RAM.users[uid_str]["phone"] = ""
...
def admin_menu_kb(uid=None):
    pairs = [
        ...
        ("tolov_usul",     "success"),
        ("auto_topup_stat","success"),
    ]
    # "Kontentdan saqlash" tugmasi bola-botda ko'rinmaydi (funksiyasi qoladi)
    if IS_CHILD_BOT:
        pairs = [(k, st) for (k, st) in pairs if k != "kontent_saqla"]
    # "Auto to'lov tushgan pul" tugmasi FAQAT bola-botda ko'rinadi
    if not IS_CHILD_BOT:
        pairs = [(k, st) for (k, st) in pairs if k != "auto_topup_stat"]
    # "Bola botga xabar yuborish" tugmasi faqat asosiy botning admin panelida
    if not IS_CHILD_BOT:
        pairs.append(("child_msg", "primary"))
...
def topup_methods_kb():
    ...
    for i, m in enumerate(manuals):
        nm = ((m or {}).get("name") or "Odiy to'lov").strip()
...
# ═══════════════════════════════════════════════════════════
# PYROGRAM (MTProto) — 2GB gacha fayllarni yuklab olish uchun
# Cloud Bot API faqat 20MB gacha yuklab oladi. API_ID/API_HASH + bot_token bilan
# Pyrogram orqali to'g'ridan-to'g'ri Telegram MTProto'ga ulanamiz — 2GB gacha.
# ═══════════════════════════════════════════════════════════
_PYRO_CLIENT = None
...
async def _get_pyrogram_client():
    global _PYRO_CLIENT, _PYRO_TRIED
    ...
    async with _PYRO_LOCK:
        ...
        try:
            ...
            logger.error(f"❌ pyrogram o'rnatilmagan: {e}. O'rnatish: pip install pyrogram tgcrypto")
            return None
        if not (API_ID and API_HASH and BOT_TOKEN):
            logger.error("❌ Pyrogram uchun API_ID/API_HASH/BOT_TOKEN to'liq emas")
            return None
        try:
            c = Client(
                name="wm_bot_dl",
            ...
            await c.start()
            _PYRO_CLIENT = c
            logger.info("✅ Pyrogram (MTProto) ulandi — 2GB gacha fayl yuklab olish mumkin")
            return _PYRO_CLIENT
        except Exception as e:
            logger.error(f"❌ Pyrogram ulanmadi: {e}")
            return None

async def _download_via_pyrogram(file_id: str, target_path: str):
    """Bot API file_id orqali MTProto'dan to'g'ridan-to'g'ri yuklab oladi (2GB gacha)."""
    cli = await _get_pyrogram_client()
    if not cli:
        return None
    try:
        # download_media file_name sifatida to'liq path qabul qiladi
        out = await cli.download_media(file_id, file_name=target_path)
        if out and os.path.exists(target_path) and os.path.getsize(target_path) > 1000:
            mb = os.path.getsize(target_path) // 1024 // 1024
            logger.info(f"✅ Pyrogram MTProto orqali yuklandi: {mb} MB → {target_path}")
            return target_path
        logger.error(f"Pyrogram yuklab oldi-yu, fayl bo'sh/yo'q: out={out}")
    except Exception as e:
        logger.error(f"❌ Pyrogram download xato: {e}")
    return None


async def _download_tg_file_safely(tg_file, target_path: str, file_id: str = ""):
    """
    Telegram'dan katta fayllarni xavfsiz yuklab oladi.
    1) Local Bot API rejimida — diskdan nusxa (lahzada)
    2) Oddiy Bot API yuklab olish (≤20MB cloud rejimida)
    3) Pyrogram MTProto fallback (20MB–2GB)
    """
    import shutil
    # 1) Local Bot API: tg_file.file_path — diskdagi absolute path bo'lishi mumkin
    try:
        local_path = getattr(tg_file, "file_path", None) or ""
        if local_path and isinstance(local_path, str):
            if os.path.isabs(local_path) and os.path.exists(local_path):
                try:
                    ...
                    logger.info(f"✅ Local Bot API fayli: {local_path} ({os.path.getsize(target_path)//1024//1024} MB)")
                    return target_path
                except Exception as e:
                    logger.warning(f"Local nusxalash xato, HTTP ga o'tildi: {e}")
    except Exception as e:
        logger.warning(f"Local fayl tekshirish xato: {e}")

    # 2) Oddiy Bot API yuklab olish
    try:
        try:
        ...
        logger.warning(f"Bot API download xato ({e}) — Pyrogram MTProto'ga o'tiladi")

    # 3) Pyrogram fallback (>20MB fayllar uchun)
    fid = file_id or getattr(tg_file, "file_id", "") or ""
    if fid:
        out = await _download_via_pyrogram(fid, target_path)
        if out:
            return out
    raise RuntimeError("Faylni yuklab bo'lmadi: Bot API ham, Pyrogram ham muvaffaqiyatsiz")
...
async def _send_original_video_fallback(bot, chat_id: int, file_id: str, caption: str,
    ...
    """
    Fallback: ffmpeg watermark muvaffaqiyatsiz bo'lsa, video oddiy caption bilan ketadi.
    Caption'ga ID/username qo'shilmaydi — watermark faqat video ICHIDA suzib chiqishi kerak.
    """
    try:
        ...
        logger.error(f"original fallback send_video xato: {e}")
        try:
            return await sm(
                bot, chat_id,
                "⚠️ Video yuborishda muammo bo'ldi. Iltimos, biroz kutib qaytadan urinib ko'ring.",
                pm=pm,
            )
        except Exception as e2:
            logger.error(f"fallback notice xato: {e2}")
            return None


def _add_watermark_ffmpeg(input_path: str, output_path: str, user_id: str, username: str = "", phone: str = "") -> bool:
    """
    ffmpeg orqali videoga majburiy watermark qo'shadi.
    Watermark: "O'g'irlash taqiqlanadi" + telefon + ID + username.
    """
    textfile_path = None
    try:
        ...
        safe_phone = str(phone or "").strip().replace("\n", " ").replace("\r", " ")[:24]
        if not safe_phone:
            safe_phone = "Tel: yo'q"
        elif not safe_phone.startswith("+"):
        ...
        # Watermark UZLUKSIZ silliq suzib yuradi (sin/cos asosida) — asosiy botdagidek.
        # Doim ko'rinadi, kichik o'lchamda, ekran bo'ylab oqib yuradi.
        x_expr = "(w-text_w-20)*(0.5+0.5*sin(2*PI*t/13))"
        y_expr = "(h-text_h-20)*(0.5+0.5*cos(2*PI*t/17))"
        ...
        # Katta (1-2 GB) videolar uchun ham watermark albatta tushishi shart:
        # birinchi urinishda sifatli (veryfast/CRF23), bo'lmasa tezkor (ultrafast/CRF28),
        # bo'lmasa eng tezkor (ultrafast/CRF32) — har biri yangi timeout bilan.
        # Tezlik birinchi o'rinda: ultrafast preset + yuqori CRF (kichikroq fayl, tezroq encode)
        attempts = [
            ("ultrafast", "28"),
            ("ultrafast", "32"),
            ("ultrafast", "36"),
        ...
        last_err = ""
        for idx, (preset, crf) in enumerate(attempts, start=1):
            cmd = [
                "ffmpeg", "-y",
                "-hide_banner", "-loglevel", "error",
                "-i", input_path,
            ...
            logger.info(f"ffmpeg watermark: urinish {idx}/{len(attempts)} (preset={preset}, crf={crf})")
            try:
                result = subprocess.run(cmd, capture_output=True, timeout=WM_FFMPEG_TIMEOUT)
            except subprocess.TimeoutExpired:
                last_err = f"timeout ({WM_FFMPEG_TIMEOUT}s)"
                logger.warning(f"ffmpeg watermark urinish {idx}: vaqt tugadi")
                continue

            if result.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 1000:
                logger.info(f"ffmpeg watermark: muvaffaqiyatli ({os.path.getsize(output_path)//1024} KB, urinish {idx})")
                return True

            last_err = result.stderr.decode("utf-8", errors="replace")[:800] if result.returncode != 0 else "output bo'sh"
            logger.warning(f"ffmpeg watermark urinish {idx} xato: {last_err[:300]}")

        logger.error(f"ffmpeg watermark: barcha urinishlar muvaffaqiyatsiz. Oxirgi: {last_err[:500]}")
        return False
...
WARN_PREFIX = ""  # Caption'ga matn qo'shilmaydi — watermark faqat video ICHIDA chiqadi

def _with_warn(caption: str) -> str:
    # Hech narsa qo'shmaymiz — caption qanday bo'lsa shunday qoldiramiz
    return caption or ""

_USER_VIDEO_LOCKS: dict = {}

def _get_user_video_lock(uid) -> asyncio.Lock:
    try:
        key = int(uid)
    except Exception:
        key = 0
    lk = _USER_VIDEO_LOCKS.get(key)
    if lk is None:
        lk = asyncio.Lock()
        _USER_VIDEO_LOCKS[key] = lk
    return lk


def _ffprobe_duration_seconds(path: str) -> float:
    """ffprobe orqali video davomiyligini (sekund) qaytaradi. Xato bo'lsa 0."""
    try:
        out = subprocess.run(
            [
                "ffprobe", "-v", "error",
        ...
        if out.returncode == 0:
            return float((out.stdout or b"").decode("utf-8", errors="replace").strip() or "0")
    except Exception as e:
        logger.warning(f"ffprobe duration xato: {e}")
    return 0.0


# Watermark faqat qisqa videolar uchun (sekund). Default: 20 daqiqa.
WM_MAX_DURATION = int(os.environ.get("WM_MAX_DURATION") or str(20 * 60))
# Ehtimoliy 20-daqiqali video o'lchami uchun proksi (Pyrogram'siz get_file uchun chegara).
# 200 MB dan katta fayllarni umuman yuklab olmaymiz — to'g'ridan-to'g'ri original yuboramiz.
WM_MAX_BYTES = int(os.environ.get("WM_MAX_BYTES") or str(200 * 1024 * 1024))


def _get_ep_meta(movie: dict, file_id: str):
    """Upload paytida saqlangan video metadata (duration, size) ni qaytaradi.
    Topilmasa (0, 0) — eski qo'shilgan qismlar uchun."""
    try:
        meta = (movie or {}).get("episode_meta", {}) or {}
        info = meta.get(file_id) or {}
        return int(info.get("duration", 0) or 0), int(info.get("size", 0) or 0)
    except Exception:
        return 0, 0
...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
                         user_id, username: str = "", markup=None, pm="HTML", protect=False, phone: str = "",
                         dur_hint: int = 0, size_hint: int = 0):
    """
    Videoni foydalanuvchiga yuboradi.
    Qoidalar:
      • Har bir foydalanuvchi uchun NAVBAT: bitta video tugamaguncha keyingisi yuborilmaydi.
      • Davomiyligi ≤ 20 daq bo'lgan videolarga ekranda suzib yuruvchi watermark
        (Tel + ID + Username) qo'shiladi.
      • Davomiyligi > 20 daq bo'lgan (yoki o'lchami juda katta) videolar
        watermarksiz, Telegram'ning o'z file_id orqali DARHOL yuboriladi
        (qayta yuklab olish/encoding YO'Q — Pyrogram MTProto bilan 2GB gacha).
      • dur_hint / size_hint berilsa (upload paytida saqlangan metadata) —
        katta/uzun videolar UMUMAN yuklab olinmasdan darhol yuboriladi (tez!).
    """
    # MAJBURIY: protect_content (faqat oddiy foydalanuvchilar uchun)
    # ── ADMIN BYPASS: adminga watermark VA protect_content QO'YILMAYDI ──
    try:
        ...
        logger.warning(f"sv_watermarked admin bypass xato: {_adm_e}")
    protect = True
    caption = _with_warn(caption)

    # ── TEZKOR QAROR: faqat DAVOMIYLIK bo'yicha ──
    # FAQAT 20 daqiqadan UZUN videolar watermarksiz, darhol original file_id bilan yuboriladi.
    # 20 daqiqadan QISQA BARCHA videolarga (metadata bo'lsa ham, bo'lmasa ham)
    # ekranda suzib yuruvchi watermark qo'shiladi.
    if dur_hint and dur_hint > WM_MAX_DURATION:
        logger.info(
            f"sv_watermarked: hint bo'yicha uzun video "
            f"({(dur_hint or 0)//60} daq > 20 daq) — watermarksiz darhol yuboriladi"
        )
    ...
    # Bitta foydalanuvchi: bir vaqtning o'zida BITTA video. Navbat bilan ishlaydi.
    user_lock = _get_user_video_lock(user_id) if user_id else None
    if user_lock is not None and user_lock.locked():
        # Foydalanuvchi avvalgi video tugashini kutadi — qisqa eslatma
        try:
            await bot.send_message(
                chat_id=chat_id,
                text="⏳ <b>Avvalgi video tugashini kuting...</b>\nNavbatga qo'shildingiz.",
                parse_mode="HTML",
            )
        except Exception:
            pass
    ...
    async def _run():
        return await _sv_watermarked_impl(
            bot, chat_id, file_id, caption,
            user_id=user_id, username=username, markup=markup, pm=pm,
            protect=protect, phone=phone,
        )
...
async def _sv_watermarked_impl(bot, chat_id: int, file_id: str, caption: str,
                               user_id, username: str = "", markup=None, pm="HTML",
                               protect=True, phone: str = ""):
    # Phone yo'q bo'lsa — RAM dan olamiz
    if not phone and user_id:
        try:
            phone = (RAM.users.get(str(user_id), {}) or {}).get("phone", "") or ""
        except Exception:
            phone = ""
    ...
    # ── 1) O'lcham bo'yicha probe (faqat log uchun). 20 daqiqadan QISQA
    #     BARCHA videolarga (o'lchami katta bo'lsa ham) suzib yuruvchi watermark
    #     qo'shiladi. Faqat DAVOMIYLIK (20 daq) qaror qiladi.
    _fsize = 0
    try:
        _probe = await asyncio.wait_for(bot.get_file(file_id), timeout=10)
        _fsize = int(getattr(_probe, "file_size", 0) or 0)
    except Exception:
        _probe = None


    if not _check_ffmpeg():
        logger.error("❌ ffmpeg topilmadi — watermarksiz original yuboriladi")
        return await _send_original_video_fallback(
            bot, chat_id, file_id, caption,
            markup=markup, pm=pm, protect=protect,
            user_id=user_id, username=username,
    ...
    # ── Progress animatsiyasi (1% → 100%) ─────────────────
    progress_msg = None
    progress_task = None
    try:
        progress_msg = await bot.send_message(
            chat_id=chat_id,
            text="📤 <b>Video yuborilmoqda...</b>\n\n▱▱▱▱▱▱▱▱▱▱  1%",
            parse_mode="HTML",
        )
    except Exception:
        progress_msg = None
    ...
    async def _animate_progress():
        ...
        try:
            while True:
                ...
                # Tezroq boshda, sekinroq 95% atrofida
                if pct < 60:
                    pct += 6
                elif pct < 85:
                    pct += 3
    ...
    try:
        async def _do_watermark():
            ...
            # Bot API get_file 20MB dan katta fayllarda "file is too big" beradi.
            # Shu sababli try/except: muvaffaqiyatsiz bo'lsa, to'g'ridan-to'g'ri Pyrogram MTProto.
            tg_file = None
            try:
                tg_file = await bot.get_file(file_id)
            except Exception as ge:
                logger.warning(f"bot.get_file xato ({ge}) — Pyrogram MTProto ishlatiladi")

            if tg_file is not None:
                await _download_tg_file_safely(tg_file, tmp_in, file_id=file_id)
            else:
                # To'g'ridan-to'g'ri Pyrogram orqali (katta fayllar uchun)
                out = await _download_via_pyrogram(file_id, tmp_in)
                if not out:
                    raise RuntimeError("Katta video yuklab olinmadi (Pyrogram ham ishlamadi). API_ID/API_HASH tekshiring.")

            logger.info(f"Watermark: {str(file_id)[:20]}... yuklab olindi ({os.path.getsize(tmp_in)//1024//1024} MB)")

            # ── DAVOMIYLIK tekshiruvi: 20 daqiqadan uzun bo'lsa watermark QO'YMAYMIZ.
            # Original videoni darhol Telegram file_id orqali yuboramiz.
            try:
                dur_sec = await asyncio.to_thread(_ffprobe_duration_seconds, tmp_in)
            except Exception:
                dur_sec = 0.0
            if dur_sec and dur_sec > WM_MAX_DURATION:
                logger.info(f"Video {int(dur_sec)//60} daq — 20 daqiqadan uzun, watermarksiz original yuboriladi")
                return await _send_original_video_fallback(
                    bot, chat_id, file_id, caption,
                    markup=markup, pm=pm, protect=protect,
                    user_id=user_id, username=username,
            ...
            logger.error("Watermark qo'yilmadi — original video fallback yuboriladi (caption watermark bilan)")
            return await _send_original_video_fallback(
                bot, chat_id, file_id, caption,
                markup=markup, pm=pm, protect=protect,
                user_id=user_id, username=username,
        ...
        # Progress animatsiyasini to'xtatamiz va 100% ko'rsatib o'chiramiz
...
async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Foydalanuvchi 📱 Kontaktni ulashish tugmasini bosganda ishlaydi."""
    ...
    # Foydalanuvchi o'z kontaktini ulashganini tekshiramiz (boshqa odamniki bo'lmasin)
    contact = msg.contact
    if contact.user_id and contact.user_id != user.id:
        await context.bot.send_message(
            chat_id=user.id,
            text="❌ Iltimos, <b>o'z</b> telefon raqamingizni ulashing.",
            parse_mode="HTML"
        )
        return
    ...
    # Reply tugmani jimgina olib tashlaymiz ("Saqlandi" yozuvisiz)
    ...
    try:
        ...
        try:
            await context.bot.delete_message(chat_id=user.id, message_id=rm_msg.message_id)
        except Exception:
            pass
    except Exception:
        pass

    # Pending start arg (masalan code_ABC) ni olamiz
    ...
    # ── MAJBURIY KANALLAR TEKSHIRUVI (kontakt ulashilgandan keyin) ──
    try:
        ns = await check_subscription(user.id, context.bot)
    except Exception as e:
        logger.error(f"contact_handler check_subscription xato: {e}")
        ns = []

    if ns:
        # Pending kodni saqlab qo'yamiz — obuna bo'lgach ishlatiladi
        if pending and pending.startswith("code_"):
            try:
                context.user_data["pending_code"] = pending.replace("code_", "").upper().strip()
            except Exception:
                pass
    ...
    # Obuna OK — agar pending code bo'lsa kino menyusini ko'rsatamiz
    if pending and pending.startswith("code_"):
        code = pending.replace("code_", "").upper().strip()
        try:
    ...
    # Aks holda — odatiy start menyusi
    context.args = []
    # Recursive start() ni _is_duplicate_update bloklamasligi uchun update_id ni reset qilamiz
    try:
        uid_upd = getattr(update, "update_id", None)
        if uid_upd is not None:
            _SEEN_UPDATE_IDS.discard(uid_upd)
    except Exception:
        pass
    try:
        await start(update, context)
    except Exception as e:
        logger.error(f"start chaqirishda xato: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── KONTAKT (telefon raqami) GATE ─────────────────────
    u_rec = RAM.ensure_user(user.id)
    if not (u_rec.get("phone") or "").strip():
        # Pending start arg (masalan code_ABC) ni saqlab qo'yamiz
        if args:
        ...
        await context.bot.send_message(
            ...
            text=("👋 <b>Xush kelibsiz!</b>\n\n"
                  "Botdan foydalanish uchun avval <b>telefon raqamingizni</b> ulashing.\n\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ══════════════════════════════════════════════
    # AUTO TO'LOV — admin "Yechib olish" tugmasi
    # ══════════════════════════════════════════════
    if data == "auto_topup_withdraw":
        if not is_any_admin(uid):
            await q.answer("⛔ Faqat admin", show_alert=True); return
        await q.answer()
        total = int(getattr(RAM, "auto_topup_total", 0) or 0)
        if total < 25000:
            await sm(context.bot, uid,
                "⚠️ <b>Pul yechish minimalkasi 25,000 so'm.</b>\n\n"
                f"💰 Sizning balansingiz: <b>{total:,} so'm</b>\n\n"
                "❗️ Ungacha pulingizni yecha olmaysiz.")
            return
        await q.answer()
        context.user_data["withdraw"] = {"step": "amount"}
        await sm(context.bot, uid,
            "💸 <b>Pul yechish</b>\n\n"
            f"💰 Mavjud balans: <b>{total:,} so'm</b>\n"
            "📉 Minimal yechish miqdori: <b>25,000 so'm</b>\n\n"
            "✍️ <b>Yechish miqdorini kiriting</b> (faqat raqam):",
            InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="withdraw_cancel")]]))
        return
    ...
    if data == "withdraw_cancel":
        ...
        try:
            await q.edit_message_text("❌ Pul yechish bekor qilindi.")
        except Exception:
            await sm(context.bot, uid, "❌ Pul yechish bekor qilindi.")
        return

    if data == "withdraw_confirm":
        if not is_any_admin(uid):
            await q.answer("⛔ Faqat admin", show_alert=True); return
        w = context.user_data.get("withdraw") or {}
        if w.get("step") != "confirm":
            await q.answer("Ma'lumot topilmadi", show_alert=True); return
        amount = int(w.get("amount") or 0)
        card   = str(w.get("card") or "")
        name   = str(w.get("name") or "")
        total  = int(getattr(RAM, "auto_topup_total", 0) or 0)
        if amount <= 0 or amount > total:
            await q.answer("Balans yetarli emas", show_alert=True)
            context.user_data.pop("withdraw", None)
            return
        commission = amount * 5 // 100
        net = amount - commission
        # Balansdan yechamiz
        RAM.auto_topup_total = total - amount
        try: save_sync()
        except Exception: pass
        context.user_data.pop("withdraw", None)
        await q.answer("✅ So'rov yuborildi")
        u = update.effective_user
        uname = ("@" + u.username) if (u and u.username) else (u.full_name if u else "Admin")
        try: uname_safe = html.escape(uname)
        except Exception: uname_safe = uname
        try: name_safe = html.escape(name)
        except Exception: name_safe = name
        # Foydalanuvchiga tasdiq
        try:
            await q.edit_message_text(
                "✅ <b>So'rov yuborildi!</b>\n\n"
                f"💵 Yechilmoqda: <b>{amount:,} so'm</b>\n"
                f"💳 Karta: <code>{card}</code>\n"
                f"👤 Karta egasi: <b>{name_safe}</b>\n"
                f"📉 Komissiya (5%): <b>{commission:,} so'm</b>\n"
                f"💰 Kartaga tushadi: <b>{net:,} so'm</b>\n\n"
                f"📊 Yangi balans: <b>{RAM.auto_topup_total:,} so'm</b>\n\n"
                "⏳ Tez orada amalga oshiriladi.",
                parse_mode="HTML"
            )
        except Exception:
            await sm(context.bot, uid,
                "✅ So'rov yuborildi! Tez orada amalga oshiriladi.")
        # Asosiy bot adminga xabar
        bot_tag = ""
        try:
            me = await context.bot.get_me()
            bot_tag = f"@{me.username}" if me.username else (me.first_name or "")
        except Exception:
            pass
        try: bot_tag_safe = html.escape(bot_tag)
        except Exception: bot_tag_safe = bot_tag
        adm_msg = (
            "💸 <b>YANGI YECHISH SO'ROVI</b>\n\n"
            f"🤖 Bot: <b>{bot_tag_safe}</b>\n"
            f"👤 Foydalanuvchi: <b>{uname_safe}</b>\n"
            f"🆔 ID: <code>{uid}</code>\n\n"
            f"💵 Yechmoqchi: <b>{amount:,} so'm</b>\n"
            f"📉 Komissiya (5%): <b>{commission:,} so'm</b>\n"
            f"💰 Kartaga tushirish kerak: <b>{net:,} so'm</b>\n\n"
            f"💳 Karta raqami: <code>{card}</code>\n"
            f"👤 Karta egasi: <b>{name_safe}</b>\n\n"
            "⚠️ Yuqoridagi kartaga aynan shuncha pul tashlang."
        )
        # Yechish so'rovini ASOSIY (parent) bot tokeni orqali asosiy adminga yuboramiz —
        # bola botning lokal adminiga emas.
        sent_parent = await notify_parent_admin(context.bot, adm_msg, parse_mode="HTML")
        if not sent_parent:
            logger.error("withdraw notify parent admin failed: all send methods failed")
        return
    ...
    if data == "topup_methods_list":
        ...
        try:
            await q.edit_message_text(
                ...
                "🌍 <i>Odiy to'lov — chek yuboriladi va admin tasdiqlaydi</i>",
                parse_mode="HTML",
    ...
    if data == "pm_add_manual" and is_any_admin(uid):
        ...
        await sm(context.bot, uid,
            "🌍 <b>Odiy to'lov usuli qo'shish</b>\n\n"
    ...
    if data.startswith("cc_check|"):
        ...
        if cc_status == "paid":
            ...
            # ✅ AUTO TO'LOV — global hisobga qo'shamiz
            RAM.auto_topup_total = int(RAM.auto_topup_total or 0) + amount
            RAM.auto_topup_count = int(RAM.auto_topup_count or 0) + 1
            await save_now()
            try:
                await q.edit_message_text(
...
async def cb_episode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Watermark bilan yuboramiz — ID/telefon videoda suzib chiqadi.
        # Progress xabari sv_watermarked ichida ko'rsatiladi (ikkita xabar bo'lmasin).
        _dh, _sh = _get_ep_meta(movie, eps[idx])
...
async def cb_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if movie:
        ...
        if 0 <= idx < len(eps):
            ...
            _dh2, _sh2 = _get_ep_meta(movie, eps[idx])
            await sv_watermarked(
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 💸 PUL YECHISH — bosqichli savol-javob ──────────
    w = context.user_data.get("withdraw")
    if w and is_any_admin(uid):
        cancel_kb = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="withdraw_cancel")]])
        step = w.get("step")
        if step == "amount":
            digits = re.sub(r"\D", "", text)
            if not digits:
                await sm(context.bot, uid, "❌ Iltimos, faqat raqam kiriting.", cancel_kb)
                return
            amount = int(digits)
            total = int(getattr(RAM, "auto_topup_total", 0) or 0)
            if amount < 25000:
                await sm(context.bot, uid,
                    f"⚠️ Minimal yechish miqdori <b>25,000 so'm</b>.\n"
                    f"Siz <b>{amount:,} so'm</b> kiritdingiz. Qaytadan kiriting:", cancel_kb)
                return
            if amount > total:
                await sm(context.bot, uid,
                    f"⚠️ Balansingizda yetarli mablag' yo'q.\n"
                    f"💰 Mavjud: <b>{total:,} so'm</b>\n"
                    f"Siz: <b>{amount:,} so'm</b> kiritdingiz.\n\n"
                    "Boshqa miqdor kiriting:", cancel_kb)
                return
            w["amount"] = amount
            w["step"] = "card"
            await sm(context.bot, uid,
                f"✅ Miqdor: <b>{amount:,} so'm</b>\n\n"
                "💳 <b>Pul tushirmoqchi bo'lgan karta raqamingizni kiriting</b>\n"
                "<i>(16 raqamli Humo/Uzcard)</i>:", cancel_kb)
            return
        if step == "card":
            digits = re.sub(r"\D", "", text)
            if len(digits) != 16:
                await sm(context.bot, uid,
                    "❌ Karta raqami noto'g'ri. <b>16 ta raqam</b> kiriting:", cancel_kb)
                return
            w["card"] = digits
            w["step"] = "name"
            await sm(context.bot, uid,
                f"✅ Karta: <code>{digits}</code>\n\n"
                "👤 <b>Karta egasining ism-familiyasini kiriting:</b>", cancel_kb)
            return
        if step == "name":
            name = text.strip()
            if len(name) < 3 or len(name) > 80:
                await sm(context.bot, uid, "❌ Ism-familiya 3-80 belgi orasida bo'lishi kerak.", cancel_kb)
                return
            w["name"] = name
            w["step"] = "confirm"
            amount = int(w.get("amount") or 0)
            ...
            confirm_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 Pulni kartga tushirish", callback_data="withdraw_confirm")],
                [InlineKeyboardButton("❌ Bekor qilish", callback_data="withdraw_cancel")],
            ])
            await sm(context.bot, uid,
                "📋 <b>Ma'lumotlarni tasdiqlang</b>\n\n"
                f"💵 Yechmoqchisiz: <b>{amount:,} so'm</b>\n"
                f"📉 Komissiya (5%): <b>{commission:,} so'm</b>\n"
                f"💰 Kartangizga tushadi: <b>{net:,} so'm</b>\n\n"
                f"💳 Karta: <code>{w['card']}</code>\n"
                f"👤 Karta egasi: <b>{name_safe}</b>\n\n"
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # Agar konflikt bo'lsa — noyob, "chiroyli" miqdor topamiz (100 ga karrali)
        conflict_amount = None
        final_amount = amount
        if amount in busy_amounts:
            conflict_amount = amount
            # Keyingi 100 ga karrali qiymatdan boshlaymiz: 1000 -> 1100, 1050 -> 1100
            candidate = ((amount // 100) + 1) * 100
            # Bo'sh va o'zidan katta bo'lgan birinchi 100-likni topamiz
            while candidate in busy_amounts:
                candidate += 100
                if candidate > amount + 5000:
                    # Juda ko'p to'qnashuv — 500 ga karrali katta offset
                    candidate = ((amount // 500) + 2) * 500
                    while candidate in busy_amounts:
                        candidate += 500
                    break
            final_amount = candidate

        # Agar konflikt bo'lsa — order OCHILMASIN, foydalanuvchidan
        # boshqa miqdor kiritishini so'raymiz (taklif bilan).
        if conflict_amount is not None:
            # Tanlangan to'lov usulini saqlab qolamiz
            if _tm:
                context.user_data["topup_method"] = _tm
            context.user_data["admin_state"] = "topup_amount"
            prompt_msg = await sm(context.bot, uid,
                f"⚠️ <b>Bu miqdor band!</b>\n\n"
                f"Ayni paytda boshqa foydalanuvchi tomonidan "
                f"<b>{conflict_amount:,} so'm</b> to'lov qilinmoqda.\n\n"
                f"Iltimos, <b>boshqa miqdor</b> kiriting.\n"
                f"Taklif: <b>{final_amount:,} so'm</b>\n\n"
                f"<i>Yangi miqdorni yuboring 👇</i>")
            if prompt_msg:
                context.user_data["topup_amount_msg_id"] = prompt_msg.message_id
            return
    ...
    if is_any_admin(uid):
        all_admin_btn_keys = [
            ...
            "tolov_usul", "factory_bots", "auto_topup_stat", "child_msg",
        ]
        ...
        if matched_key:
            ...
            if key == "child_msg":
                if IS_CHILD_BOT:
                    await sm(context.bot, uid, "❌ Bu tugma faqat asosiy botda ishlaydi.")
                    return
                context.user_data.pop("emoji_menu", None)
                context.user_data.pop("admin_state", None)
                context.user_data.pop("child_msg_target", None)
                context.user_data["child_msg_state"] = "await_id"
                await sm(context.bot, uid,
                    "📨 <b>Bola botga xabar yuborish</b>\n\n"
                    "Bola bot <b>ID</b> raqamini yuboring (bot yaratilganda berilgan raqam):\n"
    ...
    # ── Bola botga xabar — ID kiritish bosqichi ──
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_id":
        if not is_any_admin(uid):
            context.user_data.pop("child_msg_state", None)
            return
        raw = (text or "").strip().lstrip("#")
        if not raw.isdigit():
            await sm(context.bot, uid, "❌ Faqat raqam yuboring. Masalan: <code>12</code>")
            return
        try:
            row = factory_db_get(int(raw))
        except Exception as e:
            await sm(context.bot, uid, f"❌ Bazaga ulanishda xato: {e}")
            return
        if not row or (row.get("status") == "deleted"):
            context.user_data.pop("child_msg_state", None)
            await sm(context.bot, uid, "❌ Bunday ID bilan bola bot topilmadi.",
                     main_menu_kb(is_admin=is_any_admin(uid)))
            return
        if not row.get("token"):
            context.user_data.pop("child_msg_state", None)
            await sm(context.bot, uid, "❌ Bu bola botning tokeni bazada yo'q.")
            return
        context.user_data["child_msg_target"] = {
            "bid":      int(row["id"]),
            "owner_id": int(row["owner_id"]),
            "uname":    row.get("bot_username") or "",
            "token":    row["token"],
        }
        context.user_data["child_msg_state"] = "await_msg"
        await sm(context.bot, uid,
            f"✅ Bot topildi: <b>@{row.get('bot_username') or '-'}</b>\n\n"
            "Endi <b>matn, rasm yoki video</b> yuboring — xabar <b>@"
            f"{row.get('bot_username') or 'shu bola bot'}</b> orqali bot egasiga yuboriladi.\n"
            "Bekor qilish uchun /start bosing.")
        return

    # ── Bola botga xabar — matn yuborish bosqichi ──
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_msg":
        tgt = context.user_data.get("child_msg_target") or {}
        owner_id = int(tgt.get("owner_id") or 0)
        token    = tgt.get("token") or ""
        if not owner_id or not token:
            context.user_data.pop("child_msg_state", None)
            context.user_data.pop("child_msg_target", None)
            await sm(context.bot, uid, "❌ Maqsad bot yo'qoldi. Qaytadan urinib ko'ring.")
            return
        cap = (f"📨 <b>Asosiy bot adminidan xabar</b>\n"
               f"👤 Yuboruvchi: {user.full_name} (@{user.username or '-'})\n"
               f"🆔 <code>{uid}</code>\n\n{html.escape(text or '')}")
        ok, err = await _child_bot_send(token, "sendMessage",
                                        {"chat_id": owner_id, "text": cap,
                                         "parse_mode": "HTML"})
        if ok:
            await sm(context.bot, uid,
                f"✅ Xabaringiz <b>@{tgt.get('uname') or 'bola bot'}</b> orqali egasiga yuborildi!")
        else:
            await sm(context.bot, uid,
                f"❌ Yuborib bo'lmadi: {err}\n\n"
                "Bola bot ishlamayotgan yoki egasi shu bot bilan hech qachon <b>/start</b> bosmagan bo'lishi mumkin.")
        context.user_data.pop("child_msg_state", None)
        context.user_data.pop("child_msg_target", None)
        return
...
async def admin_buttons(update, context, text: str):
    ...
    if _btn_match("auto_topup_stat"):
        if not is_any_admin(uid): return
        total = int(getattr(RAM, "auto_topup_total", 0) or 0)
        cnt   = int(getattr(RAM, "auto_topup_count", 0) or 0)
        admin_user = update.effective_user
        admin_name = (admin_user.full_name if admin_user else "") or "Admin"
        try:
            admin_name_safe = html.escape(admin_name)
        except Exception:
            admin_name_safe = admin_name
        msg = (
            "<blockquote>"
            "💰 <b>AUTO TO'LOV BALANSI</b>\n\n"
            f"👤 <b>Ism:</b> {admin_name_safe}\n"
            f"💵 <b>Balans:</b> <b>{total:,} so'm</b>\n"
            f"🆔 <b>ID:</b> <code>{uid}</code>\n"
            f"💰 <b>Balans:</b> <b>{total:,} so'm</b>\n"
            f"🔢 <b>To'lovlar soni:</b> <b>{cnt:,} ta</b>\n\n"
            "<i>Faqat avtomatik (Humo/Uzcard) orqali tushgan to'lovlar hisobga olinadi.\n"
            "Cheteldan to'lov yoki adminlar tomonidan qo'lda qo'shilgan summalar bu yerda ko'rinmaydi.</i>"
            "</blockquote>"
        )
        withdraw_kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("💸 Yechib olish", callback_data="auto_topup_withdraw")
        ]])
        await sm(context.bot, uid, msg, withdraw_kb)
        return
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "pm_manual_holder":
        ...
        nm = context.user_data.pop("pm_manual_name_v", "") or "Odiy to'lov"
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid) and state == "add_ep_video":
        ...
        # ❗ Video metadata (duration + size) ni saqlaymiz —
        # yuborishda katta/uzun videolarni yuklab olmasdan darhol jo'natish uchun.
        try:
            _ep_dur = 0
            _ep_size = 0
            if msg.video:
                _ep_dur  = int(getattr(msg.video, "duration", 0) or 0)
                _ep_size = int(getattr(msg.video, "file_size", 0) or 0)
            elif msg.document:
                _ep_size = int(getattr(msg.document, "file_size", 0) or 0)
            RAM.movies[code].setdefault("episode_meta", {})[video_file_id] = {
                "duration": _ep_dur,
                "size": _ep_size,
            }
        except Exception:
    ...
    # ── Bola botga xabar — rasm/video yuborish bosqichi ──
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_msg":
        tgt = context.user_data.get("child_msg_target") or {}
        owner_id = int(tgt.get("owner_id") or 0)
        token    = tgt.get("token") or ""
        ...
        try:
            if msg.photo:
                ...
                buf.seek(0)
                ok, err = await _child_bot_send_file(
                    token, "sendPhoto", "photo", ("photo.jpg", buf.getvalue(), "image/jpeg"),
                    {"chat_id": owner_id, "caption": cap, "parse_mode": "HTML"})
            elif msg.video:
                ...
                buf.seek(0)
                ok, err = await _child_bot_send_file(
                    token, "sendVideo", "video", ("video.mp4", buf.getvalue(), "video/mp4"),
                    {"chat_id": owner_id, "caption": cap, "parse_mode": "HTML"})
            else:
                ok, err = await _child_bot_send(token, "sendMessage",
                                                {"chat_id": owner_id, "text": cap,
            ...
            await sm(context.bot, uid, f"❌ Faylni o'qishda xato: {e}")
        context.user_data.pop("child_msg_state", None)
        context.user_data.pop("child_msg_target", None)
        return
...
def _pm_main_kb():
    return ikb([
        ...
        [ibtn("🌍 Odiy to'lov qo'shish", data="pm_add_manual", style="primary")],
...
async def _handle_checkcard_webhook(data: dict):
    ...
    try:
        ...
        # ✅ AUTO TO'LOV — global hisobga qo'shamiz (webhook)
        RAM.auto_topup_total = int(RAM.auto_topup_total or 0) + pay_amount
        RAM.auto_topup_count = int(RAM.auto_topup_count or 0) + 1
...
async def _checkcard_poll_job(context):
    ...
    try:
        ...
        if cc_status == "paid":
            ...
            u_data["balance"]     = int(u_data.get("balance") or 0) + amount
            u_data["topup_total"] = int(u_data.get("topup_total") or 0) + amount
            # ✅ AUTO TO'LOV — global hisobga qo'shamiz (poll)
            RAM.auto_topup_total = int(RAM.auto_topup_total or 0) + amount
            RAM.auto_topup_count = int(RAM.auto_topup_count or 0) + 1
            await save_now()
...
def main():
    ...
    app.add_handler(MessageHandler(filters.CONTACT, contact_handler, block=False))
    ...
    async def _startup_notify(context_job):
        try:
            ...
            if IS_CHILD_BOT:
                # Bola-botlarga qisqa xabar — webhook/RAM batafsilini ko'rsatmaymiz
                try:
                    await context_job.bot.send_message(
                        ADMIN_ID,
                        "✅ <b>Botingiz ishga tushdi!</b>\n\n"
                        "Botga <code>/start</code> yuboring 👇",
                        parse_mode="HTML")
                except Exception:
                    pass
                return

            # Faqat asosiy bot — to'liq texnik xabar

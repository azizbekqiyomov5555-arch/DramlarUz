...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from telegram import WebAppInfo as _WebAppInfo

    # "Kod" tugmasi
    kod_url = kino_kanal_url or majburiy_url
    if kod_url:
        kod_btn_obj = InlineKeyboardButton(text=bt("kod_btn"), url=kod_url)
    else:
        kod_btn_obj = InlineKeyboardButton(text=bt("kod_btn"), callback_data="start_kod")

    # "Kanal" tugmasi
    kanal_url = majburiy_url or kino_kanal_url
    if kanal_url:
        kanal_btn_obj = InlineKeyboardButton(text=bt("kanal_btn"), url=kanal_url)
        row1 = [kod_btn_obj, kanal_btn_obj]
    else:
        row1 = [kod_btn_obj]

    # "Qo'llanma" tugmasi — Mini App
    qollanma_btn = InlineKeyboardButton(
        text=bt("install"),
        web_app=_WebAppInfo(url="https://azizbekqiyomov5555-arch.github.io/Kkk/")
    )

    inline_kb = InlineKeyboardMarkup([row1, [qollanma_btn]])

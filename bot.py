...
def _add_watermark_ffmpeg(input_path: str, output_path: str, user_id: str) -> bool:
    """
    ffmpeg orqali videoga 3 ta watermark qo'shadi (har doim ko'rinadi):
      • Yuqori-o'ngda:  DramlarUz botidan o'g'irlamang
      • O'rtada:        O'g'irlash taqiqlanadi (qutida)
      • Pastki-chapda:  ID: <user_id>
    ...
    try:
        ...
        tf_bot = _mk_textfile(f"ID: {user_id}")

        # Har biri har doim ko'rinadi — eng ishonchli
        common = f"{font_opt}:fontcolor=white:borderw=2:bordercolor=black@0.85"
        ...
        # Foydalanuvchi ID — ekran bo'ylab harakatlanadi (sinus/cosinus bilan),
        # har doim ko'rinadi, lekin joyi doim o'zgaradi (har joyga paydo bo'ladi)
        d3 = (
            f"drawtext=textfile='{tf_bot}':{common}"
            f":fontsize=h/20"
            f":x='(w-tw)/2 + (w-tw)/2*sin(2*PI*t/13)'"
            f":y='(h-th)/2 + (h-th)/2*cos(2*PI*t/9)'"
        )

        vf_filter = f"{d1},{d2},{d3}"

# -*- coding: utf-8 -*-
# ╔══════════════════════════════════════════════════════════════════╗
# ║  LOVABLE FACTORY WRAPPER — qo'shildi (yuqori qism)              ║
# ║  Bitta fayl 2 rejimda ishlaydi:                                  ║
# ║    • LOVABLE_ROLE=child  → asl Kino bot (pastdagi kod)           ║
# ║    • (default)           → FACTORY bot (bola botlar yasaydi)     ║
# ╚══════════════════════════════════════════════════════════════════╝
import os as _LV_os, sys as _LV_sys

def _lovable_factory_main():
    """Factory rejimi: /start → Kino bot yaratish → token → bola-bot ishga tushadi."""
    import logging, subprocess, signal, time, threading
    ...
    BOT_TOKEN    = _LV_os.environ.get("BOT_TOKEN") or ""
    ADMIN_ID     = int(_LV_os.environ.get("ADMIN_ID") or "0")
    DATABASE_URL = _LV_os.environ.get("DATABASE_URL") or ""
    SELF_FILE    = _LV_os.path.abspath(__file__)

    if not BOT_TOKEN:    raise SystemExit("❌ BOT_TOKEN kerak (factory bot tokeni)")
    if not DATABASE_URL: raise SystemExit("❌ DATABASE_URL kerak (PostgreSQL)")

    logging.basicConfig(format="%(asctime)s | %(levelname)s | FACTORY | %(message)s",
                        level=logging.INFO)
    log = logging.getLogger("factory")

    WAITING_TOKEN: set[int] = set()
    ...
    def db_insert(owner, token, uname, title):
        with db() as conn, conn.cursor() as cur:
            cur.execute("INSERT INTO factory.child_bots(owner_id,token,bot_username,bot_title,schema_name)"
                        " VALUES(%s,%s,%s,%s,%s) RETURNING id",
                        (owner, token, uname, title, "_tmp_"))
            bid = cur.fetchone()[0]
            schema = f"bot_{bid}"
    ...
    def db_user(owner):
        with db() as conn, conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM factory.child_bots WHERE owner_id=%s ORDER BY id", (owner,))
            return list(cur.fetchall())

    def db_set_status(bid, st):
        with db() as conn, conn.cursor() as cur:
            cur.execute("UPDATE factory.child_bots SET status=%s WHERE id=%s", (st, bid))
    ...
    # ─── Bola-bot subprocess ─────────────────────────────────────
    def child_db_url(schema):
        u = urlparse(DATABASE_URL)
        q = dict(parse_qsl(u.query)); q["options"] = f"-c search_path={schema}"
        return urlunparse(u._replace(query=urlencode(q)))

    def spawn(row):
        bid = row["id"]
        if bid in RUNNING and RUNNING[bid].poll() is None:
            return True
        env = _LV_os.environ.copy()
        env["LOVABLE_ROLE"] = "child"
        env["BOT_TOKEN"]    = row["token"]
        env["ADMIN_ID"]     = str(row["owner_id"])
        env["DATABASE_URL"] = child_db_url(row["schema_name"])
        env["PYTHONUNBUFFERED"] = "1"
        log_fh = open(f"child_{bid}.log", "ab")
        try:
            p = subprocess.Popen(
                [_LV_sys.executable, SELF_FILE],
                env=env, stdout=log_fh, stderr=log_fh,
                preexec_fn=_LV_os.setsid if _LV_os.name != "nt" else None,
            )
            RUNNING[bid] = p
            db_started(bid)
            log.info(f"🚀 #{bid} @{row['bot_username']} ishga tushdi (pid={p.pid})")
            return True
    ...
    def stop(bid):
        p = RUNNING.pop(bid, None)
        if not p: return
        try:
            if _LV_os.name == "nt": p.terminate()
            else: _LV_os.killpg(_LV_os.getpgid(p.pid), signal.SIGTERM)
            p.wait(timeout=5)
            ...
            try:
                for row in db_active():
                    ...
                    if p is None or p.poll() is not None:
                        log.warning(f"♻️ #{bid} qayta tushiramiz")
                        spawn(row)
            except Exception as e:
    ...
    # ─── Telegram handlers ──────────────────────────────────────
    WELCOME = ("👋 <b>Kino Bot Factory'ga xush kelibsiz!</b>\n\n"
               "Bu yerda siz <b>o'zingizning shaxsiy Kino botingizni</b> "
               "bir necha soniyada yaratishingiz mumkin. Botingizning barcha "
               "ma'lumotlari xavfsiz saqlanadi va yo'qolmaydi.\n\n"
               "Boshlash uchun pastdagi tugmani bosing 👇")

    INSTR = ("🤖 <b>Kino bot yaratish</b>\n\n"
             ...
             "(masalan: <code>mening_kino_bot</code>)\n"
             "5️⃣ BotFather sizga <b>token</b> beradi:\n   <code>1234567890:AAH...xYz</code>\n"
             "6️⃣ <b>Shu tokenni nusxa olib, pastdagi tugmani bosing va menga yuboring</b></blockquote>\n\n"
             "⚠️ <b>Diqqat:</b> Token <b>maxfiy</b> — uni hech kimga bermang!\n"
             "✅ Token to'g'ri bo'lsa, botingiz <b>darhol</b> ishga tushadi.")

    main_kb   = lambda: InlineKeyboardMarkup([
        [InlineKeyboardButton("🤖 Kino bot yaratish", callback_data="create")],
        [InlineKeyboardButton("📋 Mening botlarim",    callback_data="mybots")]])
    create_kb = lambda: InlineKeyboardMarkup([
        [InlineKeyboardButton("📤 Tokenni yuborish", callback_data="send_token")],
        [InlineKeyboardButton("⬅️ Orqaga",           callback_data="home")]])
    cancel_kb = lambda: InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="home")]])

    async def cmd_start(update, ctx):
        WAITING_TOKEN.discard(update.effective_user.id)
        await update.message.reply_text(WELCOME, reply_markup=main_kb(), parse_mode=ParseMode.HTML)

    async def on_cb(update, ctx):
        q = update.callback_query; await q.answer()
        uid, data = q.from_user.id, q.data
        if data == "home":
            WAITING_TOKEN.discard(uid)
            await q.edit_message_text(WELCOME, reply_markup=main_kb(), parse_mode=ParseMode.HTML)
        elif data == "create":
            WAITING_TOKEN.discard(uid)
            await q.edit_message_text(INSTR, reply_markup=create_kb(), parse_mode=ParseMode.HTML)
        elif data == "send_token":
            WAITING_TOKEN.add(uid)
            ...
            bots = db_user(uid)
            if not bots:
                txt = "📭 Sizda hali bot yo'q.\n«🤖 Kino bot yaratish» tugmasini bosing."
            else:
                lines = ["📋 <b>Sizning botlaringiz:</b>\n"]
                for b in bots:
                    e = "🟢" if b["status"] == "active" else "🔴"
                    lines.append(f"{e} <b>@{b['bot_username']}</b> — {b['status']} (ID: <code>{b['id']}</code>)")
                txt = "\n".join(lines)
            await q.edit_message_text(txt, reply_markup=main_kb(), parse_mode=ParseMode.HTML)

    async def on_text(update, ctx):
        uid = update.effective_user.id
        if uid not in WAITING_TOKEN:
            await update.message.reply_text("Iltimos, /start bosing.", reply_markup=main_kb())
            return
        token = (update.message.text or "").strip()
        if ":" not in token or len(token) < 20:
            await update.message.reply_text("❌ Token formati noto'g'ri.", reply_markup=cancel_kb())
            return
        msg = await update.message.reply_text("⏳ Tokenni tekshirayapman...")
        info = validate_token(token)
        if not info:
            await msg.edit_text("❌ Token noto'g'ri. BotFather'dan qaytadan oling.", reply_markup=cancel_kb())
            return
        try:
            bid = db_insert(uid, token, info.get("username",""), info.get("first_name",""))
        except psycopg2.errors.UniqueViolation:
            await msg.edit_text("⚠️ Bu token allaqachon ro'yxatdan o'tgan.", reply_markup=main_kb())
            return
        except Exception as e:
            await msg.edit_text(f"❌ Xato: {e}", reply_markup=main_kb()); return
        WAITING_TOKEN.discard(uid)
        ok = spawn({"id": bid, "owner_id": uid, "token": token,
                    "bot_username": info.get("username",""), "schema_name": f"bot_{bid}"})
        if ok:
            await msg.edit_text(
                f"✅ <b>Botingiz yaratildi!</b>\n\n🤖 <b>@{info.get('username')}</b>\n"
                f"🆔 <code>{bid}</code>\n🟢 <b>ishlamoqda</b>\n\n"
                f"Endi Telegram'da botingizga <code>/start</code> bosing!",
                reply_markup=main_kb(), parse_mode=ParseMode.HTML)
        else:
            await msg.edit_text("⚠️ Saqlandi, lekin ishga tushmadi. Admin tekshiradi.",
                                reply_markup=main_kb())

    async def cmd_stats(update, ctx):
        if update.effective_user.id != ADMIN_ID: return
        with db() as conn, conn.cursor() as cur:
            cur.execute("SELECT COUNT(*), COUNT(*) FILTER(WHERE status='active') FROM factory.child_bots")
            total, active = cur.fetchone()
        await update.message.reply_text(
            f"📊 Jami: <b>{total}</b> | Aktiv: <b>{active}</b> | "
            f"RAM'da: <b>{sum(1 for p in RUNNING.values() if p.poll() is None)}</b>",
            parse_mode=ParseMode.HTML)

    async def cmd_stop(update, ctx):
        if not ctx.args: return
        try: bid = int(ctx.args[0])
        except: return
        bots = db_user(update.effective_user.id)
        if not any(b["id"] == bid for b in bots) and update.effective_user.id != ADMIN_ID:
            await update.message.reply_text("❌ Ruxsat yo'q."); return
        stop(bid); db_set_status(bid, "stopped")
        await update.message.reply_text(f"⏹ #{bid} to'xtatildi.")

    # ─── Ishga tushirish ────────────────────────────────────────
    init_db()
    bots = db_active()
    log.info(f"📦 {len(bots)} ta aktiv bot — ishga tushiramiz...")
    for b in bots: spawn(b)
    ...
    log.info("🏭 Factory bot ishga tushdi")
    app.run_polling(allowed_updates=Update.ALL_TYPES, drop_pending_updates=True)


# ══════════════════════════════════════════════════════════════════
# DISPATCHER — qaysi rejim?
# ══════════════════════════════════════════════════════════════════
if _LV_os.environ.get("LOVABLE_ROLE") != "child":
    # FACTORY rejimi — bola-bot kodi ishga tushmaydi
    _lovable_factory_main()
    _LV_sys.exit(0)

# Aks holda — quyidagi asl KINO BOT kodi ishga tushadi (LOVABLE_ROLE=child)
# ══════════════════════════════════════════════════════════════════
# ║  ASL KINO BOT KODI (sizning bot.py — o'zgartirilmagan)           ║
# ══════════════════════════════════════════════════════════════════

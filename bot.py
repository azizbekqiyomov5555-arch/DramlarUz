...
def _build_markup(markup):
    """Dict markup ni InlineKeyboardMarkup yoki ReplyKeyboardMarkup ga aylantiradi.
    web_app, style kabi custom fieldlarni ham qo'llab-quvvatlaydi."""
    if markup is None:
        return None
    if not isinstance(markup, dict):
        return markup  # allaqachon obyekt
    from telegram import (InlineKeyboardMarkup as IKM, InlineKeyboardButton as IKB,
                          ReplyKeyboardMarkup as RKM, KeyboardButton as KB, WebAppInfo)
    if "inline_keyboard" in markup:
        ...
        for row in markup["inline_keyboard"]:
            ...
            for b in row:
                kw = {"text": b["text"]}
                if "web_app" in b:
                    kw["web_app"] = WebAppInfo(url=b["web_app"]["url"])
                elif "url" in b:
                    kw["url"] = b["url"]
                    ...
                    kw["copy_text"] = CTB(text=b["copy_text"].get("text",""))
                btns.append(IKB(**kw))
            rows.append(btns)
        return IKM(rows)
        ...
        for row in markup["keyboard"]:
            ...
            for b in row:
                if isinstance(b, dict):
                    btns.append(KB(text=b["text"]))
                else:
                    btns.append(KB(text=str(b)))
            rows.append(btns)
        return RKM(rows, resize_keyboard=markup.get("resize_keyboard", True))
    return markup


async def sm(bot, chat_id, text, markup=None, pm="HTML", reply_to_message_id=None):
    kw = {"chat_id": chat_id, "text": text, "parse_mode": pm}
    if markup: kw["reply_markup"] = _build_markup(markup)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kod_url:
        kod_btn = ibtn(bt("kod_btn"), url=kod_url, style="primary", emoji_id=get_eid("kod_btn"))
    else:
        kod_btn = ibtn(bt("kod_btn"), data="start_kod", style="primary", emoji_id=get_eid("kod_btn"))

    # "Kanal" tugmasi
    kanal_url = majburiy_url or kino_kanal_url
    if kanal_url:
        kanal_btn_item = ibtn(bt("kanal_btn"), url=kanal_url, style="primary", emoji_id=get_eid("kanal_btn"))
        start_inline_rows = [[kod_btn, kanal_btn_item]]
    else:
        start_inline_rows = [[kod_btn]]

    # "Qo'llanma" tugmasi — Telegram Mini App (web_app)
    qollanma_btn = {
        "text": bt("install"),
        "web_app": {"url": "https://azizbekqiyomov5555-arch.github.io/Kkk/"},
        "style": "success",
    }
    start_inline_rows.append([qollanma_btn])
    ...
    # Quyiga reply menyu ham yuboramiz
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "start_qollanma":
        kb = ikb([[{"text": "📖 Qo'llanmani ochish",
                    "web_app": {"url": "https://azizbekqiyomov5555-arch.github.io/Kkk/"},
                    "style": "success"}]])
        await sm(context.bot, uid, "👇 Qo'llanmani ochish uchun tugmani bosing:", kb)
        return

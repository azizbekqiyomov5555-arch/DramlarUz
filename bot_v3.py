...
async def _download_tg_file_safely(tg_file, target_path: str):
    ...
    LOCAL Bot API rejimida fayl allaqachon diskda — uni shunchaki nusxalaymiz
    (yuklab olish kerak emas). Bu 1-2 GB videolarda ham bir necha soniyada ishlaydi.
    """
    import shutil
    ...
    try:
        ...
        if local_path and isinstance(local_path, str):
            # Local Bot API absolute path qaytaradi (masalan: /var/lib/telegram-bot-api/<token>/videos/file_0.mp4)
            if os.path.isabs(local_path) and os.path.exists(local_path):
                try:
                    # Bir xil filesystemda — hardlink (lahzada)
                    ...
                    logger.info(f"✅ Local Bot API fayli to'g'ridan-to'g'ri olindi: {local_path} ({os.path.getsize(target_path)//1024//1024} MB)")
                    return target_path
                except Exception as e:
                    logger.warning(f"Local fayl nusxalashda xato, HTTP yuklab olishga o'tiladi: {e}")
    except Exception as e:
        logger.warning(f"Local fayl tekshirishda xato: {e}")

    # 2) Oddiy yuklab olish (timeout uzaytirilgan)
    try:
...
async def _send_original_video_fallback(bot, chat_id: int, file_id: str, caption: str,
    ...
    """
    Fallback: ffmpeg watermark muvaffaqiyatsiz bo'lsa ham, foydalanuvchi
    videoni KO'RA OLSIN — caption'da kuchli ogohlantirish + ID + username yoziladi.
    Shu tariqa "Video tayyorlanmadi" hech qachon chiqmaydi.
    """
    try:
        uid_txt = str(user_id) if user_id else "—"
        uname_txt = (username or "").strip()
        if uname_txt and not uname_txt.startswith("@"):
            uname_txt = "@" + uname_txt
        if not uname_txt or uname_txt == "@":
            uname_txt = "Username yo'q"

        warn_cap = (
            "⚠️ <b>O'g'irlash qat'iyan taqiqlanadi!</b>\n"
            f"🆔 ID: <code>{uid_txt}</code>\n"
            f"👤 {uname_txt}\n\n"
            f"{caption or ''}"
        )
        return await bot.send_video(
            ...
            parse_mode=pm,
            reply_markup=markup,
            protect_content=True,           # MAJBURIY himoya
            supports_streaming=True,
            read_timeout=VIDEO_IO_TIMEOUT,
            write_timeout=VIDEO_IO_TIMEOUT,
            connect_timeout=VIDEO_IO_TIMEOUT,

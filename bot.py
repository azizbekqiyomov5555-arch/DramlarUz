...
BTN_LABELS["topup_auto"]      = "Humo Uzcard (avto to'lov)"
BTN_LABELS["topup_manual"]    = "Chet eldan to'lov"
...
def topup_methods_kb():
    ...
    rows = []
    eid_auto = get_eid("topup_auto")
    eid_manual = get_eid("topup_manual")
    auto_btns = []
    for i, m in enumerate(autos):
        name = m.get("name") or f"Avto #{i+1}"
        label = f"{name} (avto)" if eid_auto else f"⚡ {name} (avto)"
        auto_btns.append(ibtn(label, data=f"topup_pick|auto|{i}", style="success", emoji_id=eid_auto))
    manual_btns = []
    for i, m in enumerate(manuals):
        label = "Chet eldan to'lov" if eid_manual else "🌍 Chet eldan to'lov"
        manual_btns.append(ibtn(label, data=f"topup_pick|manual|{i}", style="primary", emoji_id=eid_manual))
    # Juftlab joylash: chap = avto, o'ng = qo'lda
    n = max(len(auto_btns), len(manual_btns))
    for i in range(n):
        row = []
        if i < len(auto_btns):
            row.append(auto_btns[i])
        if i < len(manual_btns):
            row.append(manual_btns[i])
        rows.append(row)
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "pm_manual_holder":
        ...
        RAM.payment_methods.setdefault("manual", []).append({"card": card, "holder": holder})
        await save_now()
        context.user_data.pop("admin_state", None)
        await sm(context.bot, uid,
            f"✅ <b>Chet eldan to'lov usuli qo'shildi!</b>\n\n"
            f"💳 Karta: <code>{card}</code>\n👤 Egasi: <b>{holder}</b>",
            admin_menu_kb(uid))
        return True
...
async def _pm_show_list(bot, uid, q):
    ...
    if manuals:
        ...
        for i, m in enumerate(manuals):
            lines.append(f"  {i+1}. <b>{m.get('holder','?')}</b> — <code>{m.get('card','?')}</code>")
            rows.append([ibtn(f"🗑 🌍 {m.get('holder','?')}", data=f"pm_del|manual|{i}", style="danger")])
    if not autos and not manuals:

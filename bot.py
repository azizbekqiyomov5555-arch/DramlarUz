...
def db_initial_load():
    ...
    if DATABASE_URL and blob is None:
        ...
        if has_local:
            ...
            for _k in list(EMOJI_IDS.keys()):
                if not str(EMOJI_IDS.get(_k, "")).strip().isdigit():
                    del EMOJI_IDS[_k]
    ...
    # Bazadagi eski oddiy emoji kalitlarini RAM dan ham tozalaymiz
...
def _pe(key: str, fallback: str) -> str:
    ...
    EMOJI_IDS da faqat haqiqiy Telegram custom emoji ID (raqam) bo'ladi.
    Yo'q bo'lsa POST_EMOJI_DEFAULTS dan chiroyli oddiy emoji oladi."""
    eid = EMOJI_IDS.get(key)
    # Faqat raqamli ID bo'lsa tg-emoji ishlatamiz
    if eid and str(eid).strip().isdigit():
        return f'<tg-emoji emoji-id="{eid}">{fallback}</tg-emoji>'
    # Custom emoji yo'q — POST_EMOJI_DEFAULTS dan chiroyli oddiy emoji

...
async def _send_original_video_fallback(bot, chat_id: int, file_id: str, caption: str,
                                        markup=None, pm="HTML", protect=False):
    """
    MUHIM: Watermarksiz video YUBORILMAYDI. Bu funksiya endi faqat foydalanuvchiga
    xato xabari yuboradi — chunki har bir video majburiy ravishda foydalanuvchining
    ID/username watermark'i bilan ketishi kerak.
    """
    try:
        return await sm(
            bot, chat_id,
            "⚠️ Video tayyorlanmadi (ehtimol fayl juda katta). "
            "Iltimos, biroz kutib, qaytadan urinib ko'ring.",
            pm=pm,
        )
    except Exception as e:
        logger.error(f"watermark fallback notice xato: {e}")
        return None

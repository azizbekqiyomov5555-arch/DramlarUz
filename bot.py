...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
    ...
    """
    # MAJBURIY: protect_content
    protect = True
    caption = _with_warn(caption)

    # ── TEZKOR QAROR: metadata hint orqali (yuklab olmasdan) ──
    # Katta yoki uzun video bo'lsa — darhol original file_id bilan yuboramiz.
    if (size_hint and size_hint > WM_MAX_BYTES) or (dur_hint and dur_hint > WM_MAX_DURATION):
        logger.info(
            f"sv_watermarked: hint bo'yicha katta/uzun video "
            f"({(size_hint or 0)//1024//1024} MB, {(dur_hint or 0)//60} daq) — "
            f"watermarksiz darhol yuboriladi"
        )
    ...
    # Eski qo'shilgan videolarda duration/size saqlanmagan bo'lishi mumkin.
    # Bunday holatda bot videoni tekshirish uchun yuklab olishga urinib, 1–37% da qotib qoladi.
    # Shuning uchun metadata yo'q bo'lsa ham darhol file_id bilan original yuboramiz.
    if not dur_hint and not size_hint:
        logger.info(
            "sv_watermarked: metadata yo'q — qotib qolmasligi uchun "
            "watermarksiz darhol original file_id bilan yuboriladi"
        )
        return await _send_original_video_fallback(
            bot, chat_id, file_id, caption,
            markup=markup, pm=pm, protect=protect,
...
async def _sv_watermarked_impl(bot, chat_id: int, file_id: str, caption: str,
    ...
    # ── 1) Avval o'lcham bo'yicha tezkor qaror: juda katta bo'lsa darhol original
    _fsize = 0
    try:
    ...
    if _fsize and _fsize > WM_MAX_BYTES:
        logger.info(f"sv_watermarked: katta video ({_fsize//1024//1024} MB) — watermarksiz darhol yuboriladi")
        return await _send_original_video_fallback(
            bot, chat_id, file_id, caption,
            markup=markup, pm=pm, protect=protect,
            user_id=user_id, username=username,

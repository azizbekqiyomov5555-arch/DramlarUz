...
RAILWAY_URL = os.environ.get("RAILWAY_URL") or "https://dramlaruz-production.up.railway.app"
...
def payment_sent_kb(card: str = "", price: int = 0):
    rows = [[ibtn(bt("chek_yub"), data="send_check")]]
    copy_row = []
    if card:
        try:
            copy_row.append(CopyTextButton(text=bt("karta_nusxa"), copy_text=str(card)))
        except Exception:
            copy_row.append(ibtn(f"{bt('karta_nusxa')}: {card}", data="noop"))
    if price:
        try:
            copy_row.append(CopyTextButton(text=bt("miqdor_nusxa"), copy_text=str(price)))
        except Exception:
            copy_row.append(ibtn(f"{bt('miqdor_nusxa')}: {price}", data="noop"))
    if copy_row:
...
def topup_sent_kb(card: str = "", price: int = 0):
    ...
    rows = [[ibtn(_B("📤 Chek yuborish"), data="topup_send_check")]]
    copy_row = []
    if card:
        try:
            copy_row.append(CopyTextButton(text=bt("karta_nusxa"), copy_text=str(card)))
    ...
    if data.startswith("cc_check|"):
        ...
        if cc_status == "paid":
            ...
            try:
                ...
                lichka_kb_cc = ikb([[ibtn("👤 Foydalanuvchi lichkasi", url=lichka_url)]])
                await context.bot.send_message(
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        if card and card != "Karta raqami o'rnatilmagan":
            try:
                ...
                copy_row.append(ibtn(f"{bt('karta_nusxa')}: {card}", data="noop"))
        try:
            copy_row.append(CopyTextButton(text=bt("miqdor_nusxa"), copy_text=str(final_amount)))
        except Exception:
            copy_row.append(ibtn(f"{bt('miqdor_nusxa')}: {final_amount}", data="noop"))
        kb_rows = []
        if copy_row:
            kb_rows.append(copy_row)
        kb_rows.append([ibtn("❌ Bekor qilish", data=f"cc_cancel|{pid}")])
        ...
        # Webhook handler ham o'chira olishi uchun RAM ga ham saqlaymiz
        if topup_msg_id:
            RAM.pending_payments[pid]["topup_msg_id"] = topup_msg_id
...
async def _handle_checkcard_webhook(data: dict):
    ...
    try:
        ...
        if _BOT_APP and _BOT_APP.bot:
            ...
            topup_msg_id = found_pay.get("topup_msg_id")
            if topup_msg_id:
                try:
                    await _BOT_APP.bot.delete_message(chat_id=int(uid_str), message_id=topup_msg_id)
                except Exception:
                    pass
            try:
            ...
            try:
                ...
                lichka_kb_wh = ikb([[ibtn("👤 Foydalanuvchi lichkasi", url=lichka_url)]])
                await _BOT_APP.bot.send_message(
                    ADMIN_ID, adm_cap, parse_mode="HTML",
    ...
    try:
        ...
        if cc_status == "paid":
            ...
            try:
                ...
                lichka_kb_poll = ikb([[ibtn("👤 Foydalanuvchi lichkasi", url=lichka_url)]])
                await context.bot.send_message(
                    ADMIN_ID, adm_cap, parse_mode="HTML",

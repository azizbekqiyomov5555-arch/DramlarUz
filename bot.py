...
def db_initial_load():
    ...
    if DATABASE_URL and blob is None:
        ...
        if has_local:
            ...
            # Bazada yo'q post emojilari uchun default qiymatlar
            for _k, _v in POST_EMOJI_DEFAULTS.items():
                EMOJI_IDS.setdefault(_k, _v)
...
def _pe(key: str, fallback: str) -> str:
    ...
    Agar EMOJI_IDS da custom emoji bo'lsa — uni ishlatadi.
    Yo'q bo'lsa POST_EMOJI_DEFAULTS dan default emoji oladi."""
    eid = EMOJI_IDS.get(key)
    if eid:
        return f'<tg-emoji emoji-id="{eid}">{fallback}</tg-emoji>'
    # Custom emoji yo'q — POST_EMOJI_DEFAULTS dan default emoji

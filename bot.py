...
def _escape_drawtext_text(value: str) -> str:
    """drawtext text= uchun xavfsiz escape."""
    value = str(value or "")
    return (
        value.replace('\\', '\\\\')
             .replace(':', '\\:')
...
def _add_watermark_ffmpeg(input_path: str, output_path: str, user_id: str, username: str = "") -> bool:
    ...
    Watermark video bo'ylab har xil joyda paydo bo'lib yo'qoladi,
    lekin bir vaqtda faqat bitta yozuv ko'rinadi va video o'lchami o'zgarmaydi.
    """
    try:
        ...
        if not safe_username:
            safe_username = "nomi yo'q"

        wm_text = f"O'g'irlash taqiqlanadi\\nID: {user_id} | {safe_username}"
        wm_text = _escape_drawtext_text(wm_text)

        font_path = _find_font()
        font_opt = f":fontfile='{font_path}'" if font_path else ""

        # 4 soniyalik sikl: 2.8 soniya ko'rinadi, 1.2 soniya yo'qoladi.
        # Har siklda joyi almashadi. Overlay video o'lchamini o'zgartirmaydi.
        cycle = 4
        visible = f"lt(mod(t\\,{cycle})\\,2.8)"
        step = f"mod(floor(t/{cycle})\\,7)"
        x_expr = (
            f"if(eq({step}\\,0)\\,(w-text_w)/2\\,"
            f"if(eq({step}\\,1)\\,w*0.06\\,"
            f"if(eq({step}\\,2)\\,w-text_w-w*0.06\\,"
            f"if(eq({step}\\,3)\\,w*0.06\\,"
            f"if(eq({step}\\,4)\\,w-text_w-w*0.06\\,"
            f"if(eq({step}\\,5)\\,(w-text_w)/2\\,(w-text_w)/2))))))"
        )
        y_expr = (
            f"if(eq({step}\\,0)\\,(h-text_h)/2\\,"
            f"if(eq({step}\\,1)\\,h*0.08\\,"
            f"if(eq({step}\\,2)\\,h*0.08\\,"
            f"if(eq({step}\\,3)\\,h-text_h-h*0.10\\,"
            f"if(eq({step}\\,4)\\,h-text_h-h*0.10\\,"
            f"if(eq({step}\\,5)\\,h*0.42\\,h*0.78))))))"
        )

        vf_filter = (
            f"drawtext=text='{wm_text}'"
            f"{font_opt}"
            f":fontsize='max(18\\,min(w\\,h)/28)'"
        ...
        logger.info("ffmpeg watermark: bitta harakatlanuvchi yozuv qo'shilmoqda")
...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
    ...
    try:
        async def _do_watermark():
            ...
            success = await asyncio.to_thread(_add_watermark_ffmpeg, tmp_in, tmp_out, str(user_id), str(username or ""))
        ...
        # ── 90 soniya timeout — hamma qismlarda watermark ulgurishi uchun ──
        return await asyncio.wait_for(_do_watermark(), timeout=90.0)

    except asyncio.TimeoutError:
        logger.warning(f"sv_watermarked: 90s timeout — originalni yuboramiz")
        try:
            return await sv(bot, chat_id, file_id, caption, markup, pm, protect)

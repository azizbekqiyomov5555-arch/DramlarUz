...
def main_menu_kb(is_admin=False):
    ...
    # Faqat asosiy botda: bola botga xabar yuborish
    if not IS_CHILD_BOT:
        rows.append([rbtn(bt("child_msg"), style="primary")])
    if is_admin:
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (not IS_CHILD_BOT) and _main_btn("child_msg"):
    ...
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_id":
        ...
        await sm(context.bot, uid,
            ...
            "Endi <b>matn, rasm yoki video</b> yuboring — admin (egasiga) yetkaziladi.\n"
    ...
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_msg":
        ...
        if not owner_id:
            ...
            return
        cap = (f"📨 <b>Sizning botingizga xabar</b>\n"
               f"👤 Yuboruvchi: {user.full_name} (@{user.username or '-'})\n"
               f"🆔 <code>{uid}</code>\n\n{text}")
        try:
            await context.bot.send_message(owner_id, cap, parse_mode="HTML")
            await sm(context.bot, uid, "✅ Xabaringiz bola bot adminiga yuborildi!")
        except Exception as e:
            await sm(context.bot, uid, f"❌ Yuborib bo'lmadi: {e}\n\n"
                     "Iltimos admin avval sizning asosiy botingiz bilan <b>/start</b> bosgan bo'lishi kerak.")
        context.user_data.pop("child_msg_state", None)
        context.user_data.pop("child_msg_target", None)
        return
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (not IS_CHILD_BOT) and context.user_data.get("child_msg_state") == "await_msg":
        ...
        try:
            if msg.photo:
                await context.bot.send_photo(owner_id, msg.photo[-1].file_id,
                                             caption=cap, parse_mode="HTML")
            elif msg.video:
                await context.bot.send_video(owner_id, msg.video.file_id,
                                             caption=cap, parse_mode="HTML")
            else:
                await context.bot.send_message(owner_id, cap, parse_mode="HTML")
            await sm(context.bot, uid, "✅ Xabaringiz bola bot adminiga yuborildi!")

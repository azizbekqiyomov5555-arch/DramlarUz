...
def rbtn(text, style=None, emoji_id=None, web_app=None):
    b = {"text": text}
    if style:    b["style"] = style
    if emoji_id: b["icon_custom_emoji_id"] = emoji_id
    if web_app:  b["web_app"] = {"url": web_app}
...
def _to_markup(markup):
    """Dict markup ni python-telegram-bot obyektiga aylantiradi (web_app ni ham qo'llab-quvvatlaydi)."""
    ...
    if "keyboard" in markup:
        ...
        for row in markup["keyboard"]:
            btns = []
            for b in row:
                if isinstance(b, dict) and "web_app" in b:
                    btns.append(KB(text=b["text"], web_app=WebAppInfo(url=b["web_app"]["url"])))

    ...
    def ensure_user(self, uid):
        ...
        with self._lock:
            ...
            if "referrer_id" not in u:
                u["referrer_id"] = None
            if "referred_users" not in u or not isinstance(u.get("referred_users"), list):
                u["referred_users"] = []
...
BTN_LABELS["ref_narx"] = "Referral narxi"
...
def balans_kb():
    ...
    return ikb([
        ...
        [
            ibtn("🎁 Do'st taklif qilish", data="ref_share", style="success"),
        ],
    ])
...
def clear_admin_state(context):
    for key in [
        ...
        "admin_balance_target", "price_ep_range", "ref_price_input",
    ]:
...
# REFERRAL HELPER
# ══════════════════════════════════════════════════════════
async def _maybe_credit_referrer(bot, new_user_id) -> bool:
    """Yangi foydalanuvchi barcha majburiy kanallarga obuna bo'lganda
    referrer balansiga referral_amount qo'shadi. Bir martagina ishlaydi."""
    try:
        uid_str = str(new_user_id)
        u = RAM.ensure_user(uid_str)
        ...
        try:
            ...
            return False
        if ref_id_int == int(new_user_id):
            return False
        # Yangi foydalanuvchi haqiqatan barcha kanallarga obuna bo'lganmi?
        ns = await check_subscription(new_user_id, bot)
        if ns:
            return False
        ref_owner = RAM.get_user(str(ref_id_int))
        if not ref_owner:
            return False
        # Egasi bloklangan bo'lsa berilmaydi
        if is_blocked_user(ref_id_int):
            u["referral_credited"] = True
            await save_now()
            return False
        amount = int((RAM.settings or {}).get("referral_amount") or 200)
        if amount <= 0:
            u["referral_credited"] = True
            await save_now()
            return False
        # Egasiga to'lov
        owner = RAM.ensure_user(str(ref_id_int))
        if uid_str in (owner.get("referred_users") or []):
            # Allaqachon hisobga olingan
            u["referral_credited"] = True
            await save_now()
            return False
        owner["balance"] = int(owner.get("balance") or 0) + amount
        owner["referral_earnings"] = int(owner.get("referral_earnings") or 0) + amount
        owner.setdefault("referred_users", []).append(uid_str)
        u["referral_credited"] = True
        await save_now()
        # Egasiga xabar
        try:
            new_user_obj = RAM.get_user(uid_str) or {}
            new_name = new_user_obj.get("name") or f"ID: {uid_str}"
            await bot.send_message(
                ref_id_int,
                f"🎉 <b>Sizning do'stingiz botdan ro'yxatdan o'tdi!</b>\n\n"
                f"👤 Do'st: <b>{new_name}</b>\n"
                f"🎁 Sizga: <b>+{amount:,} so'm</b> berildi!\n"
                f"💰 Joriy balans: <b>{owner['balance']:,} so'm</b>\n\n"
                f"<i>Yana do'st taklif qiling va pulli dramalarni sotib oling!</i>",
                parse_mode="HTML")
        except Exception:
            pass
        # Yangi foydalanuvchiga ham eslatma
        try:
            await bot.send_message(
                int(new_user_id),
                f"✅ Siz do'stingiz havolasi orqali qo'shildingiz.\n"
                f"🎁 Do'stingizga <b>{amount:,} so'm</b> berildi.",
                parse_mode="HTML")
        except Exception:
            pass
        return True
    except Exception as e:
        logger.warning(f"_maybe_credit_referrer xato: {e}")
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].startswith("ref_"):
        try:
            ref_id_raw = args[0].replace("ref_", "").strip()
            if ref_id_raw.isdigit() and int(ref_id_raw) != int(user.id):
                existed_before = RAM.get_user(user.id) is not None
                u_data = RAM.ensure_user(user.id)
                already_registered = bool(
                    existed_before and (
                        u_data.get("referral_credited")
                        or u_data.get("referrer_id")
                        or (RAM.get_user(user.id) or {}).get("first_seen")
                    )
                )
                # Faqat yangi (hech kim taklif qilmagan) foydalanuvchini biriktiramiz
                if not u_data.get("referrer_id") and not u_data.get("referral_credited"):
                    ref_owner = RAM.get_user(ref_id_raw)
                    if ref_owner:
                        u_data["referrer_id"] = ref_id_raw
                elif already_registered:
                    try:
                        await context.bot.send_message(
                            chat_id=user.id,
                            text=("✅ <b>Siz allaqachon botdan ro'yxatdan o'tgansiz.</b>\n\n"
            ...
            pass
        # Code emas — oddiy menyuga davom etamiz (pastdagi ns check ishga tushadi)
    ...
    if args and args[0].startswith("code_"):
        ...
        if ns:
            ...
            return
        # Kanallarga obuna — referral kreditini berishga harakat qilamiz
        try:
            await _maybe_credit_referrer(context.bot, user.id)
        except Exception as _e:
            logger.warning(f"ref credit error: {_e}")
    ...
    # ── REFERRAL CREDIT (obunadan keyin) ──────────────────
    try:
        await _maybe_credit_referrer(context.bot, user.id)
    except Exception as _e:
        logger.warning(f"ref credit error: {_e}")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ref_share":
        try:
            ...
            ref_price = int((RAM.settings or {}).get("referral_amount") or 200)
            try:
                bot_me = await context.bot.get_me()
                bot_uname = bot_me.username or ""
            except Exception:
                bot_uname = ""
            ref_link = f"https://t.me/{bot_uname}?start=ref_{uid}" if bot_uname else ""
            share_text = (
                f"🎬 Eng zo'r dramalar shu yerda!\n"
                f"Botga qo'shilish uchun havolani bosing 👇\n{ref_link}")
            from urllib.parse import quote as _quote
            tg_share = f"https://t.me/share/url?url={_quote(ref_link)}&text={_quote(share_text)}"
            kb_rows = [
                [ibtn("📤 Do'stga ulashish", url=tg_share, style="success")],
            ]
            if ref_link:
                kb_rows.append([{"text": "📋 Havolani nusxalash",
                                  "copy_text": {"text": ref_link}}])
            kb = ikb(kb_rows)
            await sm(context.bot, uid,
                f"🎁 <b>Do'st taklif qilib pul ishlang!</b>\n\n"
                f"Siz do'stingizni taklif qiling va <b>{ref_price:,} so'm</b> oling, "
                f"keyin pullik dramalarni sotib oling!\n\n"
                f"👥 Taklif qilingan: <b>{ref_count} ta</b>\n"
                f"💰 Ishlangan: <b>{ref_earn:,} so'm</b>\n\n"
                f"🔗 <b>Sizning havolangiz:</b>\n<code>{ref_link}</code>\n\n"
                f"<i>Do'stingiz havolangiz orqali botga kirib, barcha majburiy "
                f"kanallarga obuna bo'lganda — sizga avtomatik {ref_price:,} so'm beriladi. "
                f"Har bir do'st uchun faqat bir marta.</i>",
                kb)
        except Exception as _e:
            await q.answer(f"Xato: {_e}", show_alert=True)
        return

    if data == "admin_set_ref_price":
        if not is_any_admin(uid) or not has_perm(uid, "ref_narx"):
            await q.answer("⛔ Ruxsat yo'q", show_alert=True)
            return
        context.user_data["admin_state"] = "ref_price_input"
        await sm(context.bot, uid,
            "🎁 <b>Referral narxini o'zgartirish</b>\n\n"
            "Yangi summani so'mda yuboring (masalan: <code>500</code>):\n"
            "<i>0 — referral mukofotini o'chiradi.</i>")
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _main_btn("balans"):
        ...
        try:
            bot_me_x = await context.bot.get_me()
            bot_uname_x = bot_me_x.username or ""
        except Exception:
            bot_uname_x = ""
        ref_link_x = f"https://t.me/{bot_uname_x}?start=ref_{uid}" if bot_uname_x else f"start=ref_{uid}"
        txt = (
            f"💰 <b>Balansingiz</b>\n\n"
            f"👤 Ism: <b>{name}</b>\n"
            f"🆔 ID: <code>{uid}</code>\n\n"
            ...
            f"🎁 Referraldan ishlangan: <b>{ref_earn:,} so'm</b>\n"
            f"💎 Har bir do'st uchun: <b>{ref_price:,} so'm</b>\n\n"
            f"🔗 <b>Sizning referal silkangiz:</b>\n"
            f"<code>{ref_link_x}</code>\n\n"
            f"<i>Do'stingizni taklif qiling va pullik dramalarni sotib oling!</i>"
        )
...
async def admin_buttons(update, context, text: str):
    ...
    if _btn_match("ref_narx"):
        if not has_perm(uid, "ref_narx"):
            await sm(context.bot, uid, "⛔ Sizda bu huquq yo'q.", admin_menu_kb(uid))
            return
        cur = int((RAM.settings or {}).get("referral_amount") or 200)
        kb = ikb([[ibtn("✏️ O'zgartirish", data="admin_set_ref_price", style="success")]])
        await sm(context.bot, uid,
            f"🎁 <b>Referral mukofoti</b>\n\n"
            f"Hozirgi narx: <b>{cur:,} so'm</b>\n\n"
            f"<i>Har bir taklif qilingan do'st majburiy kanallarga obuna bo'lganda "
            f"taklif qilgan foydalanuvchi balansiga shu summa qo'shiladi. "
            f"Bir do'st uchun faqat bir marta beriladi.</i>",
            kb)
        return
...
async def admin_state_handler(update, context, text: str) -> bool:
    ...
    if state == "ref_price_input":
        ...
        if not t.isdigit():
            await sm(context.bot, uid, "❌ Faqat raqam kiriting. Masalan: <code>500</code>")
            return True
        new_amount = int(t)
        if new_amount < 0 or new_amount > 10_000_000:
            await sm(context.bot, uid, "❌ Noto'g'ri qiymat. 0 dan 10 000 000 gacha bo'lsin.")
            return True
        RAM.settings["referral_amount"] = new_amount
        await save_now()
        clear_admin_state(context)
        await sm(context.bot, uid,
            f"✅ Referral mukofoti yangilandi!\n\n"
            f"🎁 Yangi narx: <b>{new_amount:,} so'm</b>",
            admin_menu_kb(uid))
        return True
    ...
    if state == "block_user_input":
        ...
        ref_count_t = len(u_data.get("referred_users") or [])
        ref_earn_t  = int(u_data.get("referral_earnings") or 0)
        ref_by_t    = u_data.get("referrer_id") or "—"
        ref_info    = (f"\n👥 Taklif qilganlari: <b>{ref_count_t} ta</b>"
                       f"\n🎁 Referral pul: <b>{ref_earn_t:,} so'm</b>"
                       f"\n🔗 Uni taklif qilgan: <code>{ref_by_t}</code>")
        if already_blocked:
            ...
            await sm(context.bot, uid,
                ...
                f"💰 Balans: <b>{balance:,} so'm</b>{premium_str}{ref_info}\n\n"

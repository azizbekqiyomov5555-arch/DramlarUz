...
def ibtn(text, data=None, url=None, style=None, emoji_id=None, web_app=None):
    b = {"text": text}
...
def _to_markup(markup):
    """Faqat inline keyboard da web_app bo'lsa WebAppInfo ga aylantiradi.
    Reply keyboard va boshqa dict larni to'g'ridan-to'g'ri qaytaradi."""
    if markup is None or not isinstance(markup, dict):
        return markup
    # Reply keyboard — to'g'ridan-to'g'ri (style, emoji fieldlari saqlansin)
    if "keyboard" in markup:
        return markup
    # Inline keyboard — faqat web_app bo'lsa convert qilamiz
    if "inline_keyboard" in markup:
        ...
        if not has_webapp:
            return markup  # web_app yo'q — o'zgartirishsiz qaytaramiz
        from telegram import InlineKeyboardMarkup as IKM, InlineKeyboardButton as IKB, WebAppInfo
        rows = []
        for row in markup["inline_keyboard"]:
            btns = []
            for b in row:
...
async def sm(bot, chat_id, text, markup=None, pm="HTML", reply_to_message_id=None):
    ...
    if markup: kw["reply_markup"] = _to_markup(markup)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # "Qo'llanma" tugmasi — Mini App sifatida ochiladi
    start_inline_rows.append([
        ibtn(bt("install"), web_app="https://azizbekqiyomov5555-arch.github.io/Kkk/",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "start_qollanma":
        await sm(context.bot, uid,
                 "👇 Qo'llanmani ochish uchun tugmani bosing:",
                 ikb([[ibtn(bt("install"), web_app="https://azizbekqiyomov5555-arch.github.io/Kkk/",
                            style="success", emoji_id=get_eid("install"))]]))
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _main_btn("install"):
        await context.bot.send_message(
            chat_id=uid,
            text="\u200b",
            reply_markup=_to_markup(ikb([[ibtn(bt("install"),
                web_app="https://azizbekqiyomov5555-arch.github.io/Kkk/",
                style="success", emoji_id=get_eid("install"))]])))
        return

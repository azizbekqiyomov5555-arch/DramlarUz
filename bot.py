...
async def _send_original_video_fallback(bot, chat_id: int, file_id: str, caption: str,
                                        markup=None, pm="HTML", protect=False,
                                        user_id=None, username: str = "", force: bool = False):
    """
    Original videoni (watermarksiz) yuboradi.
    QOIDA: oddiy foydalanuvchiga watermarksiz video ASLO yuborilmaydi.
    Faqat: (a) admin, (b) force=True (20 daqiqadan UZUN videolar uchun).
    Aks holda — xato xabari ko'rsatiladi va qayta urinishga taklif qilinadi.
    """
    _is_admin = False
    try:
        _is_admin = int(user_id or 0) == ADMIN_ID
    except Exception:
        _is_admin = False

    if not (_is_admin or force):
        # Watermarksiz yuborishga RUXSAT YO'Q — foydalanuvchiga eslatma
        logger.warning(
            f"⛔ Watermarksiz yuborish RAD ETILDI (user={user_id}). "
            f"Foydalanuvchiga qayta urinish taklif qilinadi."
        )
        try:
            return await sm(
                bot, chat_id,
                "⚠️ <b>Video tayyorlanmoqda...</b>\n\n"
                "Iltimos, 10-20 soniyadan keyin qaytadan qismni bosing.\n"
                "<i>(Video xavfsizlik belgisi bilan yuboriladi)</i>",
                pm="HTML",
            )
...
# Source (raw downloaded) file cache: file_id -> local path
# Shu file_id'ni bitta foydalanuvchi yuklab olgan bo'lsa, keyingilari qayta yuklab olmaydi.
_SRC_CACHE: dict = {}
_SRC_CACHE_MAX = 40

def _src_cache_get(fid: str):
    try:
        p = _SRC_CACHE.get(str(fid))
        if p and os.path.exists(p) and os.path.getsize(p) > 1000:
            return p
        if p:
            _SRC_CACHE.pop(str(fid), None)
    except Exception:
        pass
    return None

def _src_cache_put(fid: str, path: str):
    try:
        if not path or not os.path.exists(path):
            return
        if len(_SRC_CACHE) >= _SRC_CACHE_MAX:
            # eng eski fayllarni o'chiramiz
            for k in list(_SRC_CACHE.keys())[: max(1, _SRC_CACHE_MAX // 4)]:
                old = _SRC_CACHE.pop(k, None)
                try:
                    if old and os.path.exists(old):
                        os.unlink(old)
                except Exception:
                    pass
        # Asl fayl o'chmasligi uchun nusxa olamiz
        import shutil
        fd, dst = tempfile.mkstemp(suffix=".mp4", prefix="wm_src_cache_")
        os.close(fd)
        shutil.copyfile(path, dst)
        _SRC_CACHE[str(fid)] = dst
    except Exception as e:
        logger.warning(f"src cache put xato: {e}")


# ═══════════════════════════════════════════════════════════════
# 🚀 BACKGROUND PRE-WARM WORKER
# Bot bo'sh paytida (foydalanuvchi yo'q) barcha kinolar/qismlar uchun
# manba (source) faylni oldindan yuklab _SRC_CACHE ga joylaydi.
# Foydalanuvchi qismni bosganda — yuklab olish kerak emas, faqat
# ffmpeg watermark (10-20 soniya) qolаdi. Tezkor javob beradi.
# ═══════════════════════════════════════════════════════════════
_PREWARM_STARTED = False
_PREWARM_DONE_FIDS: set = set()

async def _prewarm_episode_source(bot, file_id: str):
    """Bitta qism uchun source faylni yuklab _SRC_CACHE ga joylaydi."""
    try:
        if not file_id or str(file_id) in _PREWARM_DONE_FIDS:
            return
        if _src_cache_get(file_id):
            _PREWARM_DONE_FIDS.add(str(file_id))
            return
        # Probe — juda katta (>20 daq) bo'lsa o'tkazib yuboramiz
        try:
            tg_file = await asyncio.wait_for(bot.get_file(file_id), timeout=20)
        except Exception:
            tg_file = None

        fd, tmp_path = tempfile.mkstemp(suffix=".mp4", prefix="prewarm_")
        os.close(fd)
        try:
            if tg_file is not None:
                await _download_tg_file_safely(tg_file, tmp_path, file_id=file_id)
            else:
                ok = await _download_via_pyrogram(file_id, tmp_path)
                if not ok:
                    raise RuntimeError("pyrogram download failed")

            # Davomiyligi 20 daq dan uzun bo'lsa — cache qilmaymiz (watermark qo'yilmaydi)
            try:
                dur_sec = await asyncio.to_thread(_ffprobe_duration_seconds, tmp_path)
            except Exception:
                dur_sec = 0.0
            if dur_sec and dur_sec > WM_MAX_DURATION:
                logger.info(f"prewarm: {str(file_id)[:18]}... uzun ({int(dur_sec)//60} daq) — o'tkazildi")
                try: os.unlink(tmp_path)
                except: pass
                _PREWARM_DONE_FIDS.add(str(file_id))
                return

            _src_cache_put(file_id, tmp_path)
            _PREWARM_DONE_FIDS.add(str(file_id))
            logger.info(f"✅ prewarm: {str(file_id)[:18]}... ({os.path.getsize(tmp_path)//1024//1024} MB) cache'landi")
        finally:
            try: os.unlink(tmp_path)
            except: pass
    except Exception as e:
        logger.warning(f"prewarm xato ({str(file_id)[:18]}...): {e}")


async def _prewarm_loop(bot):
    """
    Doimiy fonda ishlaydigan worker.
    Har 30 soniyada barcha kinolardagi qismlarni tekshiradi va
    cache'da yo'qlarini sekin-sekin yuklab oladi.
    """
    global _PREWARM_STARTED
    if _PREWARM_STARTED:
        return
    _PREWARM_STARTED = True
    logger.info("🚀 Prewarm worker ishga tushdi")
    await asyncio.sleep(30)  # bot to'liq ishga tushishini kutish
    while True:
        try:
            # Barcha kinolardagi file_id larni yig'amiz
            all_fids = []
            try:
                for code, movie in (RAM.movies or {}).items():
                    for ep_fid in (movie.get("episodes", []) or []):
                        if ep_fid and str(ep_fid) not in _PREWARM_DONE_FIDS:
                            all_fids.append(ep_fid)
            except Exception:
                pass
            ...
            # Cache to'lib ketmasin — eng ko'pi 30 ta yangi qism
            for fid in all_fids[:30]:
                # Foydalanuvchi videosi ishlatilayotgan bo'lsa, prewarm to'xtaydi
                if _WM_SEMAPHORE.locked():
                    await asyncio.sleep(15)
                    continue
                try:
                    await _prewarm_episode_source(bot, fid)
                except Exception as e:
                    logger.warning(f"prewarm episode xato: {e}")
                # Tarmoqqa yuk bo'lmasin
                await asyncio.sleep(8)

            await asyncio.sleep(120)
        except Exception as e:
            logger.error(f"prewarm loop xato: {e}")
            await asyncio.sleep(60)
...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
    ...
    """
    # Admin uchun: watermark YO'Q, protect YO'Q — saqlash/skrinshot/forward erkin
    try:
        _is_admin_user = int(user_id or 0) == ADMIN_ID
    except Exception:
        _is_admin_user = False
    if _is_admin_user:
        try:
            return await bot.send_video(
                chat_id=chat_id, video=file_id, caption=(caption or ""),
                parse_mode=pm, reply_markup=markup,
                protect_content=False, supports_streaming=True,
            )
        except Exception as _ae:
            logger.warning(f"admin direct send xato: {_ae}")
    # MAJBURIY: protect_content (oddiy foydalanuvchilar uchun)
    protect = True
    ...
    if dur_hint and dur_hint > WM_MAX_DURATION:
        ...
        return await _send_original_video_fallback(
            ...
            user_id=user_id, username=username, force=True,
        )
...
async def _sv_watermarked_impl(bot, chat_id: int, file_id: str, caption: str,
    ...
    if cached_fid:
        try:
            _is_adm2 = False
            try: _is_adm2 = int(user_id or 0) == ADMIN_ID
            except Exception: _is_adm2 = False
            return await bot.send_video(
                chat_id=chat_id, video=cached_fid, caption=caption,
                parse_mode=pm, reply_markup=markup,
                protect_content=(not _is_adm2),
                supports_streaming=True,
            )
    ...
    try:
        async def _do_watermark():
            ...
            # Source cache: shu file_id avval yuklab olingan bo'lsa, qayta yuklab olmaymiz
            _cached_src = _src_cache_get(file_id)
            if _cached_src:
                try:
                    import shutil
                    shutil.copyfile(_cached_src, tmp_in)
                    logger.info(f"src cache HIT: {str(file_id)[:20]}... ({os.path.getsize(tmp_in)//1024//1024} MB)")
                except Exception as _ce:
                    logger.warning(f"src cache copy xato: {_ce}")
                    _cached_src = None
            ...
            if not _cached_src:
                ...
                # Yangi yuklab olindi — keyingi foydalanuvchilar uchun cache qilamiz
                try:
                    _src_cache_put(file_id, tmp_in)
                except Exception:
                    pass
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "auto_topup_withdraw":
        ...
        if total <= 0:
            ...
            return
        await q.answer(
            f"💸 Yechib olish so'rovi qabul qilindi.\nMiqdor: {total:,} so'm\n\nTez orada amalga oshiriladi.",
            show_alert=True
        )
        return
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_any_admin(uid):
        all_admin_btn_keys = [
            ...
            "tolov_usul", "factory_bots", "auto_topup_stat",
        ]
...
async def admin_buttons(update, context, text: str):
    ...
    if _btn_match("auto_topup_stat"):
        ...
        msg = (
            ...
            "<i>Faqat avtomatik (Humo/Uzcard CheckCard) orqali tushgan to'lovlar hisobga olinadi.\n"
...
def main():
    ...
    if app.job_queue:
        ...
        # 🚀 Background prewarm worker — barcha qismlarni oldindan tayyorlaydi
        async def _start_prewarm(context_job):
            try:
                asyncio.create_task(_prewarm_loop(context_job.bot))
            except Exception as e:
                logger.warning(f"prewarm ishga tushmadi: {e}")
        if not IS_CHILD_BOT:
            app.job_queue.run_once(_start_prewarm, when=45)
            logger.info("🚀 Background prewarm yoqildi (45s dan keyin)")

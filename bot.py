...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        card_sent_msg = await sm(context.bot, uid,
            f"✅ <b>To'lov yaratildi!</b>\n\n"
            f"{amount_line}"
            f"📋 Order ID: <code>{cc_order}</code>\n\n"
            f"{card_line}"
            f"⬆️ <b>Yuqoridagi karta raqamiga aynan shu miqdorni o'tkazing!</b>\n"
            f"<i>To'lovni amalga oshirgach bot avtomatik ravishda tekshirib, balansingizni to'ldiradi.</i>\n\n"
            f"<blockquote>⚠️ DIQQAT: {final_amount:,} so'mdan kam yoki ko'p tashlasangiz to'lov qabul qilinmaydi!</blockquote>\n"
            f"⏱ To'lov <b>15 daqiqa</b> ichida amalga oshirilishi kerak.",
            topup_auto_kb)
        # Karta xabari message_id ni saqlaymiz — to'lov tushganda o'chiriladi
        RAM.pending_payments[pid]["card_msg_id"] = card_sent_msg.message_id
        ...
        if context.application.job_queue:
            context.application.job_queue.run_repeating(
                ...
                interval=15,
                first=15,
                data={"pid": pid, "uid": uid, "order": cc_order, "amount": final_amount, "tries": 0},
                name=f"cc_poll_{pid}",
            )
        return
...
async def _handle_checkcard_webhook(data: dict):
    ...
    try:
        ...
        if _BOT_APP and _BOT_APP.bot:
            # Karta xabarini o'chiramiz
            card_msg_id = found_pay.get("card_msg_id")
            if card_msg_id:
                try:
                    await _BOT_APP.bot.delete_message(chat_id=int(uid_str), message_id=card_msg_id)
                except Exception:
                    pass
            try:
...
async def _checkcard_poll_job(context):
    ...
    try:
        ...
        if cc_status == "paid":
            ...
            await save_now()
            # Karta xabarini o'chiramiz
            card_msg_id = pay.get("card_msg_id")
            if card_msg_id:
                try:
                    await context.bot.delete_message(chat_id=uid, message_id=card_msg_id)
                except Exception:
                    pass
            try:
                await context.bot.send_message(

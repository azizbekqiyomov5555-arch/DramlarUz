    ...
    try:
        async def _do_watermark():
            ...
            if success and os.path.exists(tmp_out) and os.path.getsize(tmp_out) > 1000:
                # Videoni Telegram native video sifatida ko'rsatish uchun
                # width / height / duration / thumbnail / supports_streaming kerak.
                width, height, duration = _probe_video_info(tmp_out)
                thumb_path = _make_video_thumb(tmp_out)
                try:
                    kw = {
                        "chat_id": chat_id,
                        "caption": caption,

...
def topup_methods_kb():
    ...
    for i, m in enumerate(autos):
        ...
        rows.append([ibtn(f"⚡ {name} (avto)", data=f"topup_pick|auto|{i}", style="success")])
    for i, m in enumerate(manuals):
        holder = m.get("holder") or "Chet eldan to'lov"
        rows.append([ibtn(f"🌍 {holder} (qo'lda)", data=f"topup_pick|manual|{i}", style="primary")])
    ...
    if data == "topup_methods_list":
        ...
        try:
            await q.edit_message_text(
                ...
                "🌍 <i>Qo'lda to'lov — chet eldan, chek yuboriladi va admin tasdiqlaydi</i>",
                parse_mode="HTML",

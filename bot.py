...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "auto_topup_withdraw":
        ...
        if total <= 0:
            await q.answer("Balansingiz 0 so'm — yechib olish uchun pul yo'q", show_alert=True)
            return
        if total < 25000:
            await q.answer(
                f"⚠️ Minimal yechish miqdori 25,000 so'm.\nSizning balansingiz: {total:,} so'm",
                show_alert=True
            )
            return
        await q.answer()
    ...
    if data == "withdraw_confirm":
        ...
        try:
            await context.bot.send_message(ADMIN_ID, adm_msg, parse_mode="HTML")
        except Exception as e:
            logger.error(f"withdraw notify admin failed: {e}")
        return

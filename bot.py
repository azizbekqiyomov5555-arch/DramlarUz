...
def main():
    ...
    # ── ExtBot KLASSINI patch qilamiz (instance emas!) ──────────────
    # __slots__ muammosini chetlab o'tadi — klass metodlari override qilinadi
    from telegram.ext import ExtBot as _ExtBot

    _orig_send      = _ExtBot.send_message
    _orig_edit      = _ExtBot.edit_message_text
    _orig_photo     = _ExtBot.send_photo
    _orig_video     = _ExtBot.send_video

    async def _p_send(self, *a, **kw):
        if kw.get("parse_mode") == "HTML" and "text" in kw:
            kw["text"] = _e(kw["text"])
        return await _orig_send(self, *a, **kw)

    async def _p_edit(self, *a, **kw):
        if kw.get("parse_mode") == "HTML" and "text" in kw:
            kw["text"] = _e(kw["text"])
        return await _orig_edit(self, *a, **kw)

    async def _p_photo(self, *a, **kw):
        if kw.get("parse_mode") == "HTML" and "caption" in kw:
            kw["caption"] = _e(kw["caption"])
        return await _orig_photo(self, *a, **kw)

    async def _p_video(self, *a, **kw):
        if kw.get("parse_mode") == "HTML" and "caption" in kw:
            kw["caption"] = _e(kw["caption"])
        return await _orig_video(self, *a, **kw)

    _ExtBot.send_message      = _p_send
    _ExtBot.edit_message_text = _p_edit
    _ExtBot.send_photo        = _p_photo
    _ExtBot.send_video        = _p_video
    logger.info("✅ Premium emoji patch qo'llandi (klass darajasida)")

...
# ─── KONFIGURATSIYA ────────────────────────────────────────
BOT_TOKEN  = os.environ.get("BOT_TOKEN")  or "8723400610:AAE5bz5FPDLuEccQ5xzOOQePSrOybiXY3NE"
...
_raw_railway_url = os.environ.get("RAILWAY_URL") or "dramlaruz-production.up.railway.app"
RAILWAY_URL = _raw_railway_url if _raw_railway_url.startswith("https://") else f"https://{_raw_railway_url}"
...
# ─── DEFAULT PREMIUM EMOJI IDlari (Telegram built-in animated emoji) ──────────
# Bu IDlar Telegram o'zining premium animated emoji pack laridan olingan.
# Admin sozlamalar orqali o'zgartirish mumkin, lekin default sifatida
# shu IDlar ishlatiladi — hamma yerda premium animated emoji ko'rinadi.
DEFAULT_EMOJI_IDS: dict = {
    "✅": "5368324170671202286",
    "❌": "5447644880824181073",
    "⚠":  "5467666648263564704",
    "⚠️": "5467666648263564704",
    "🚀": "5431815452437257407",
    "🟢": "5287654073106775789",
    "🔴": "5287560751596068295",
    "💰": "5472308992514464083",
...
# ─── XABAR MATNIDAGI ODDIY EMOJIlarni PREMIUM ga AVTOMATIK ALMASHTIRISH ───────
import re as _re
_EMOJI_RE_AUTO = _re.compile(
...
def _e(text: str) -> str:
    """
    Xabar matni ichidagi oddiy (Unicode) emojilari ni
    Telegram premium animated emoji ga avtomatik almashtiradi.
    Faqat DEFAULT_EMOJI_IDS da bor emojilarga tegadi, qolganlar o'zgarishsiz.
    """
    def _replace(m):
        ch = m.group(0)
        # Avvalo EMOJI_IDS (admin sozlagan) dan, keyin DEFAULT_EMOJI_IDS dan qidiradi
        eid = EMOJI_IDS.get(ch) or DEFAULT_EMOJI_IDS.get(ch)
        if eid:
            return f'<tg-emoji emoji-id="{eid}">{ch}</tg-emoji>'
        return ch
    return _EMOJI_RE_AUTO.sub(_replace, text)
...
def _pe(key: str, fallback: str) -> str:
    ...
    Admin sozlagan EMOJI_IDS dan, yo'q bo'lsa DEFAULT_EMOJI_IDS dan qidiradi."""
    eid = EMOJI_IDS.get(key) or DEFAULT_EMOJI_IDS.get(fallback)
    if eid:
...
async def sm(bot, chat_id, text, markup=None, pm="HTML", reply_to_message_id=None):
    # Barcha oddiy emojilari premium animated emoji ga avtomatik o'giriladi
    if pm == "HTML":
        text = _e(text)
...
async def qe(q, text, parse_mode="HTML", reply_markup=None):
    """q.edit_message_text uchun wrapper — premium emoji avtomatik qo'llanadi."""
    ...
    if data.startswith("cc_check|"):
        ...
        if cc_status == "paid":
            ...
            try:
                ...
                logger.warning(f"cc_check admin notify xato: {_exc}")
    ...
    if price > 0 and not is_episode_paid(user_id, code, ep):
        ...
        if balance >= price:
            ...
            try:
                ...
                logger.warning(f"Admin balance notify xato: {_exc}")
...
async def cb_buy_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.warning(f"Admin buy_all notify xato: {_exc}")
    ...
    try:
        ...
        if _BOT_APP and _BOT_APP.bot:
            ...
            try:
                ...
                logger.warning(f"Webhook admin notify xato: {_exc}")
    except Exception as e:
    ...
    try:
        ...
        if cc_status == "paid":
            ...
            try:
                ...
                logger.warning(f"Poll admin notify xato: {_exc}")
...
def main():
    ...
    # ── Bot metodlarini premium emoji uchun wrap qilamiz ──
    # Barcha context.bot.send_message / edit_message_text chaqiruvlarida
    # HTML parse_mode bo'lsa — oddiy emojilari premium emoji ga aylanadi
    _orig_send_message      = app.bot.send_message
    _orig_edit_message_text = app.bot.edit_message_text
    _orig_send_photo        = app.bot.send_photo
    _orig_send_video        = app.bot.send_video

    async def _patched_send_message(*args, **kwargs):
        if kwargs.get("parse_mode") == "HTML" and "text" in kwargs:
            kwargs["text"] = _e(kwargs["text"])
        elif len(args) >= 2 and isinstance(args[1], str) and kwargs.get("parse_mode") == "HTML":
            args = list(args)
            args[1] = _e(args[1])
        return await _orig_send_message(*args, **kwargs)

    async def _patched_edit_message_text(*args, **kwargs):
        if kwargs.get("parse_mode") == "HTML" and "text" in kwargs:
            kwargs["text"] = _e(kwargs["text"])
        elif len(args) >= 1 and isinstance(args[0], str) and kwargs.get("parse_mode") == "HTML":
            args = list(args)
            args[0] = _e(args[0])
        return await _orig_edit_message_text(*args, **kwargs)

    async def _patched_send_photo(*args, **kwargs):
        if kwargs.get("parse_mode") == "HTML" and "caption" in kwargs:
            kwargs["caption"] = _e(kwargs["caption"])
        return await _orig_send_photo(*args, **kwargs)
    ...
    # PTB v21+ __setattr__ bloklaydi — __dict__ orqali bypass qilamiz
    app.bot.__dict__['send_message']      = _patched_send_message
    app.bot.__dict__['edit_message_text'] = _patched_edit_message_text
    app.bot.__dict__['send_photo']        = _patched_send_photo
    app.bot.__dict__['send_video']        = _patched_send_video
    logger.info("✅ Premium emoji monkey-patch qo'llandi — barcha HTML xabarlarda premium emoji ishlaydi")

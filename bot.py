...
def _compress_video_to_telegram_limit(input_path: str):
    """Video sifatini buzmaslik uchun majburiy siqmaydi; katta fayl document bo'lib yuboriladi."""
    try:
        original_size = os.path.getsize(input_path)
        if original_size <= TELEGRAM_SAFE_UPLOAD_LIMIT_BYTES:
            return input_path, None
        logger.warning(
            f"Video {original_size // 1024 // 1024}MB — sifat va o'lcham buzilmasligi uchun "
            "qayta siqilmaydi, kerak bo'lsa document sifatida yuboriladi"
        )

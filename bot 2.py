...
def _add_watermark_ffmpeg(input_path: str, output_path: str, user_id: str) -> bool:
    ...
    ffmpeg orqali videoga watermark qo'shadi:
      • DramlarUz botidan o'g'irlamang / O'g'irlash taqiqlanadi — doim ko'rinadi
      • Har bir foydalanuvchining Telegram ID raqami videoning turli joylarida
        paydo bo'lib, bir necha soniyadan keyin yo'qoladi.
    Matnlar textfile orqali beriladi — apostrof muammosi yo'q.
    """
    tmp_files = []
    try:
        font_path = _find_font()
        if not font_path:
            logger.error("❌ Hech qanday TTF font topilmadi — watermark imkonsiz")
            return False
        font_opt = f"fontfile='{font_path}'"

        # ── Matn fayllarini yaratamiz (har bir drawtext uchun alohida) ──
        def _mk_textfile(txt: str) -> str:
            fd, p = tempfile.mkstemp(suffix=".txt", prefix="wm_")
            os.close(fd)
        ...
        tf_top = _mk_textfile("DramlarUz botidan o'g'irlamang")
        tf_mid = _mk_textfile("O'g'irlash taqiqlanadi")
        tf_id = _mk_textfile(f"Telegram ID: {user_id}")

        # O'qilishi uchun oq matn + qora kontur
        common = f"{font_opt}:fontcolor=white:borderw=3:bordercolor=black@0.90"

        # Yuqori-o'ng burchak
        d1 = (
            f"drawtext=textfile='{tf_top}':{common}"
            f":fontsize=h/26:x=w-tw-12:y=12"
        )
        # O'rta (yarim shaffof quti bilan)
        d2 = (
            f"drawtext=textfile='{tf_mid}':{font_opt}"
            f":fontcolor=white:fontsize=h/18"
            f":box=1:boxcolor=black@0.45:boxborderw=10"
            f":x=(w-tw)/2:y=h*0.45"
        )
        # ID har 12 soniyada navbat bilan turli joylarda 3 soniyadan chiqadi.
        # Shu sabab har bir foydalanuvchiga aynan o'z Telegram ID si video ichida ko'rinadi.
        id_common = f"drawtext=textfile='{tf_id}':{common}:fontsize=h/18:box=1:boxcolor=black@0.32:boxborderw=8"
        id_layers = [
            f"{id_common}:x=24:y=24:enable='between(mod(t,12),0,3)'",
            f"{id_common}:x=w-tw-24:y=24:enable='between(mod(t,12),3,6)'",
            f"{id_common}:x=24:y=h-th-24:enable='between(mod(t,12),6,9)'",
            f"{id_common}:x=w-tw-24:y=h-th-24:enable='between(mod(t,12),9,12)'",
            f"{id_common}:x=(w-tw)/2:y=(h-th)/2:enable='between(mod(t+4,17),0,2.5)'",
        ]

        vf_filter = ",".join([d1, d2, *id_layers])
        ...
        logger.info(f"ffmpeg ishga tushdi (user={user_id})")
        result = subprocess.run(cmd, capture_output=True, timeout=240)

        if result.returncode != 0:
            err = result.stderr.decode("utf-8", errors="replace")
            logger.error(f"ffmpeg xato (rc={result.returncode}):\n{err[-1200:]}")
            return False

        if not os.path.exists(output_path) or os.path.getsize(output_path) < 2000:
            logger.error("ffmpeg: chiqish fayli juda kichik yoki yo'q")
            return False

        logger.info("✅ Watermark muvaffaqiyatli qo'shildi")
        return True

    except subprocess.TimeoutExpired:
        logger.error("ffmpeg: 4 daqiqa timeout")
        return False
    except Exception as e:
...
async def sv_watermarked(bot, chat_id: int, file_id: str, caption: str,
    ...
    5 daqiqadan oshsa — originalni yuborib, kutmaymiz.
    """
    ...
    try:
        ...
        # ── 5 daqiqa timeout — uzun qismlar ham yetib chiqsin ──
        return await asyncio.wait_for(_do_watermark(), timeout=300.0)

    except asyncio.TimeoutError:
        logger.warning(f"sv_watermarked: 5 daq timeout — originalni yuboramiz")
        try:
            return await sv(bot, chat_id, file_id, caption, markup, pm, protect)
    ...
    if movie:
        ...
        if 0 <= idx < len(eps):
            await asyncio.gather(
                ...
                sv_watermarked(context.bot, pay["user_id"], eps[idx],

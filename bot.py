...
def topup_methods_kb():
    ...
    for i, m in enumerate(autos):
        ...
        auto_btns.append(ibtn(f"⚡ {name} (avto)", data=f"topup_pick|auto|{i}", style="success"))
    manual_btns = []
    for i, m in enumerate(manuals):
        holder = m.get("holder") or "Chet eldan to'lov"
        manual_btns.append(ibtn(f"🌍 {holder}", data=f"topup_pick|manual|{i}", style="primary"))
